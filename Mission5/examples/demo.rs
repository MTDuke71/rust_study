...
fn set_operations_demo() {
    ...
    let programming_langs = SetOperations::from_iter([
        "Rust", "Python", "JavaScript", "Go", "Java"
    ]);
    
    let systems_langs = SetOperations::from_iter([
        "Rust", "C", "C++", "Go", "Zig"
    ]);
    ...
    println!("Is {{Rust, Go}} subset of programming? {}", 
             small_set.is_subset(&programming_langs));
    println!("Are programming and systems disjoint? {}", 
...
fn coordinate_mapping_demo() {
    ...
    if let Some((start_x, start_y)) = start_pos {
        ...
        println!("Empty positions adjacent to start: {:?}", adjacent_empty.to_vec());
    }
}