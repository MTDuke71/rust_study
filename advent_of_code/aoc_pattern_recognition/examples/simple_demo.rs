...
fn demo_grid_patterns() {
    ...
    println!("  Pattern: {} (complexity varies by implementation)", 
...
fn demo_state_patterns() {
    ...
    println!("  Cache: {} hits, {} misses, {:.1}% hit rate", hits, misses, hit_rate * 100.0);
    ...
    if let Some((start, length)) = tracker.find_cycle() {
        println!("  🔄 Cycle detected: starts at gen {}, length {}", start, length);
    }
}
