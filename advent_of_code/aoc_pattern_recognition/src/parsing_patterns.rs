...
use crate::{PatternComplexity, PatternError, PatternResult};
    ...
    fn parse_line(&self, line: &str) -> PatternResult<Coordinate> {
        ...
        if parts.len() >= 2 {
            let x = parts[0].parse::<i32>().map_err(|_| {
                PatternError::InvalidInput(format!("Invalid x coordinate: {}", parts[0]))
            })?;
            let y = parts[1].parse::<i32>().map_err(|_| {
                PatternError::InvalidInput(format!("Invalid y coordinate: {}", parts[1]))
            })?;

            Ok(Coordinate { x, y })
        } else {
            Err(PatternError::InvalidInput(
                "Could not find x,y coordinates".to_string(),
            ))
        }
    }
...
impl ParsePattern<(String, String)> for KeyValueParser {
    fn parse_line(&self, line: &str) -> PatternResult<(String, String)> {
        ...
        if parts.len() == 2 {
            ...
            Err(PatternError::InvalidInput(format!(
        ...
        for line in input.lines() {
            ...
            if trimmed.starts_with('[') && trimmed.ends_with(']') {
                ...
                if let Some(ref section) = self.current_section {
                    ...
                    return Err(PatternError::InvalidInput(
                        "Content found before section header".to_string(),
                    ));
                }
            }
        }
...
impl ParsingPatternRecognizer {
    ...
    fn has_coordinate_pattern(&self, lines: &[&str]) -> bool {
        lines.iter().any(|line| {
            line.contains(',') && line.chars().filter(|c| c.is_ascii_digit()).count() >= 2
        })
    }
    ...
    fn has_instruction_pattern(&self, lines: &[&str]) -> bool {
        lines.iter().any(|line| {
            ...
            words.len() >= 2 && words[0].chars().all(|c| c.is_ascii_alphabetic())
        })
    }
...
pub mod parse_utils {
    ...
    pub fn parse_range(text: &str) -> PatternResult<(i32, i32)> {
        if let Some(pos) = text.find("..") {
            ...
            Err(PatternError::InvalidInput(
                "No range delimiter found".to_string(),
            ))
        }
    }
}
