    ...
    pub fn find_all(&self, target: &T) -> Vec<Coord> {
        self.cells
            ...
            .filter_map(|(coord, value)| {
                if value == target { Some(*coord) } else { None }
            })
            .collect()
    }
...
impl AocPattern for ShortestPathPattern {
    ...
    fn solve(&self, input: Self::Input) -> PatternResult<Self::Output> {
        ...
        if !grid.in_bounds(&start) || !grid.in_bounds(&end) {
            return Err(PatternError::InvalidInput("Start or end position out of bounds".to_string()));
        }
...
impl AocPattern for FloodFillPattern {
    ...
    fn solve(&self, input: Self::Input) -> PatternResult<Self::Output> {
        ...
        if !grid.in_bounds(&start) {
            return Err(PatternError::InvalidInput("Start position out of bounds".to_string()));
        }
        
        let start_char = grid.get(&start).ok_or_else(|| {
            PatternError::InvalidInput("No cell at start position".to_string())
        })?;
