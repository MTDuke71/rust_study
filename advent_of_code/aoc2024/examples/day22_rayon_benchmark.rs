...
fn benchmark_part2(buyers: &[i64], _iterations: usize) {
    ...
    println!(
        "Serial:              {:?} → Result: {}",
        serial_time, result_serial
    );
    println!(
        "Parallel (Mutex):    {:?} → Result: {}",
        parallel_time, result_parallel
    );
    println!(
        "Parallel (Optimized): {:?} → Result: {}",
        optimized_time, result_optimized
    );
    ...
    assert_eq!(
        result_serial, result_optimized,
        "Optimized result must match serial!"
    );
...
fn main() {
    ...
    let input =
        std::fs::read_to_string("inputs/day22_example.txt").expect("Failed to read input file");
    ...
    println!(
        "   - Total PRNG operations: {} × 2000 × 3 steps = {} operations",
        buyers.len(),
        buyers.len() * 2000 * 3
    );
