use aoc2024::solver::day20::{bfs_distances, parse_input, Pos};
use mission6::Grid;
...
fn reconstruct_path(distances: &HashMap<Pos, usize>, start: Pos, end: Pos) -> Vec<Pos> {
    ...
    while current != start {
        ...
        for next in [
            Pos {
                row: current.row.wrapping_sub(1),
                col: current.col,
            },
            Pos {
                row: current.row + 1,
            ...
            Pos {
                row: current.row,
                col: current.col.wrapping_sub(1),
            },
            Pos {
                row: current.row,
                col: current.col + 1,
    ...
    for row in 0..grid.height() {
        for col in 0..grid.width() {
            ...
            if let Some((cheat_start, cheat_end, _)) = cheat {
                ...
                if cheat_start.manhattan_distance(&pos)
                    <= cheat_start.manhattan_distance(&cheat_end)
                    && cheat_end.manhattan_distance(&pos)
                        <= cheat_start.manhattan_distance(&cheat_end)
                {
                    let dist_sum =
                        cheat_start.manhattan_distance(&pos) + pos.manhattan_distance(&cheat_end);
...
fn main() {
    let input =
        std::fs::read_to_string("inputs/day20_example.txt").expect("Failed to read input file");
    ...
    visualize_grid(
        ...
        &path,
        None,
        &format!("Normal Path (Time: {} ps)", normal_time),
    );
