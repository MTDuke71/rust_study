...
fn example_reduce() {
    ...
    // Parallel reduce
    let sum_parallel = numbers.par_iter().map(|&x| x).reduce(
...
fn example_into_par_iter() {
    ...
    println!(
        "✅ Into_par_iter: Processed {} robots",
        final_positions.len()
    );
}
...
fn example_when_to_parallelize() {
    ...
    println!(
        "  Speedup:  {:.2}x",
        serial_large.as_secs_f64() / parallel_large.as_secs_f64()
    );
}
...
fn example_par_extend() {
    ...
    println!(
        "✅ Par_extend: Generated {} robots in parallel",
        robots.len()
    );
}
