    ...
    F: FnMut(i64) -> i64,
{
    ...
    println!("{:30} {:>12} ops in {:>10?} ({:>8.2} ns/op)",
             name,
             iterations,
             elapsed,
...
fn main() {
    ...
    println!("   └─> Power-of-2 is {:.2}x FASTER (AND vs DIV)\n", mod_speedup);
    ...
    println!("   └─> Power-of-2 is {:.2}x FASTER (SHL vs IMUL)\n", mul_speedup);
    ...
    println!("   └─> Power-of-2 is {:.2}x FASTER (SAR vs IDIV)\n", div_speedup);
    ...
    println!("   Operations: {} × {} = {} total evolutions", secrets, evolutions, secrets * evolutions);
    ...
    println!("\n🚀 SPEEDUP: Power-of-2 is {:.2}x FASTER than prime version!", prng_speedup);
    ...
    println!("═══════════════════════════════════════════════════════════════");
    println!();
    println!("1️⃣  MODULO: AND instruction is ~{:.0}x faster than DIV", mod_speedup);
    println!("   • Power-of-2: val & 0xFFFFFF (1 cycle)");
    println!("   • Prime: val % 16777213 (~20-40 cycles)");
    println!();
    println!("2️⃣  MULTIPLICATION: SHL is ~{:.0}x faster than IMUL", mul_speedup);
    ...
    println!();
    println!("4️⃣  COMBINED EFFECT: {:.2}x slowdown for prime-based PRNG", prng_speedup);
    println!("   • Day 22 got LUCKY with power-of-2 constants!");
    println!("   • If AoC used primes, solution would be {}x SLOWER", prng_speedup as i32);
    println!();
    ...
    println!("   cargo asm -p aoc2024 --example day22_prime_vs_power2_benchmark --release modulo_power2");
    println!("   cargo asm -p aoc2024 --example day22_prime_vs_power2_benchmark --release modulo_prime");
}
