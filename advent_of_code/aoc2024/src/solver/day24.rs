...
fn parse_input(input: &str) -> (HashMap<String, u8>, Vec<Gate>) {
    ...
    if sections.len() != 2 {
        panic!(
            "Expected 2 sections separated by blank line, got {}",
            sections.len()
        );
    }
...
fn extract_z_value(wire_values: &HashMap<String, u8>) -> i64 {
    ...
    z_wires.sort_by_key(|(name, _)| name[1..].parse::<usize>().unwrap());
...
fn find_swapped_wires(gates: &[Gate], _gate_by_output: &HashMap<String, &Gate>) -> Vec<String> {
    ...
    println!(
        "\n[Rule 1] Z-wires must be XOR outputs (except {})",
        final_z
    );
    for gate in gates {
        if gate.output.starts_with('z') && gate.output != final_z && gate.gate_type != GateType::Xor
        {
            println!(
                "  ⚠ {} produced by {:?} instead of XOR",
                gate.output, gate.gate_type
            );
    ...
    for gate in gates {
        if gate.gate_type == GateType::Xor {
            let has_xy_input = gate.input1.starts_with('x')
                || gate.input1.starts_with('y')
                || gate.input2.starts_with('x')
    ...
    for gate in gates {
        if gate.gate_type == GateType::Xor {
            ...
            if is_xy_xor && gate.output.starts_with('z') && gate.output != "z00" {
                println!(
                    "  ⚠ {} = {} XOR {} (first-level XOR shouldn't output to z)",
            ...
            if is_xy_xor && gate.output != "z00" {
                ...
                if !found_xor_consumer {
                    if let Some(ctype) = consumer_type {
                        println!(
                            "  ⚠ {} (from {} XOR {}) consumed by {:?} instead of XOR",
                            gate.output, gate.input1, gate.input2, ctype
                        );
                    } else {
                        println!(
                            "  ⚠ {} (from {} XOR {}) not consumed by any gate",
                            gate.output, gate.input1, gate.input2
                        );
                    }
                    suspicious.push(gate.output.clone());
    ...
    for wire in suspicious {
        ...
        if let Some(gate) = gate_by_output.get(wire) {
            ...
            if wire.starts_with('z') && gate.gate_type != GateType::Xor {
                println!(
                    "\n{} is z-wire from {:?}, should be from XOR",
                    wire, gate.gate_type
                );
                ...
                for candidate in suspicious {
                    ...
                    if let Some(cand_gate) = gate_by_output.get(candidate) {
                        ...
                        if cand_gate.gate_type == GateType::Xor {
                            let has_xy = cand_gate.input1.starts_with('x')
                                || cand_gate.input1.starts_with('y')
                                || cand_gate.input2.starts_with('x')
                                || cand_gate.input2.starts_with('y');
                            ...
                            if !has_xy && !to_z {
                                ...
                                println!(
                                    "  → Candidate swap partner: {} (orphaned XOR)",
                                    candidate
                                );
            ...
            // Case 2: Orphaned XOR - should output to a z-wire
                ...
                let has_xy = gate.input1.starts_with('x')
                    || gate.input1.starts_with('y')
                    || gate.input2.starts_with('x')
                    || gate.input2.starts_with('y');
                ...
                if !has_xy && !to_z {
                    println!(
                        "\n{} is orphaned XOR (inputs: {}, {})",
                        wire, gate.input1, gate.input2
                    );
                    ...
                    for candidate in suspicious {
                        ...
                        if candidate.starts_with('z') {
                            if let Some(cand_gate) = gate_by_output.get(candidate) {
                                if cand_gate.gate_type != GateType::Xor {
                                    println!(
                                        "  → Candidate swap partner: {} (z-wire from {:?})",
                                        candidate, cand_gate.gate_type
                                    );
                                    pairs.push((wire.clone(), candidate.clone()));
                                    used.insert(wire.clone());
                                    used.insert(candidate.clone());
            ...
            // Case 3: AND output not feeding OR - might be swapped with carry chain wire
                ...
                for candidate in suspicious {
                    ...
                    if let Some(cand_gate) = gate_by_output.get(candidate) {
                        if cand_gate.gate_type == GateType::Xor {
                            let is_xy_xor = (cand_gate.input1.starts_with('x')
                                || cand_gate.input1.starts_with('y'))
                                && (cand_gate.input2.starts_with('x')
                                    || cand_gate.input2.starts_with('y'));
                            ...
                            if is_xy_xor {
                                println!(
                                    "  → Candidate swap partner: {} (XOR from x,y)",
                                    candidate
                                );
                                pairs.push((wire.clone(), candidate.clone()));
                                used.insert(wire.clone());
