            ...
            if let Some(neighbors_b) = graph.get(node_b) {
                for node_c in neighbors_b {
                    ...
                    if neighbors_a.contains(node_c) {
                        ...
                        if node_a.starts_with('t')
                            || node_b.starts_with('t')
                            || node_c.starts_with('t')
                        {
...
fn bron_kerbosch(
    ...
    if p.is_empty() && x.is_empty() {
        println!(
            "🎯 BASE CASE! Found maximal clique of size {}: {:?}",
            r.len(),
            r.iter().cloned().collect::<Vec<_>>()
        );
    ...
    let candidates: Vec<String> = if let Some(ref pivot_node) = pivot {
        ...
        println!(
            "   Candidates after pivot: {} nodes (was {} before pivot)",
            p.difference(&pivot_neighbors).count(),
            p.len()
        );
        cands
    } else {
    ...
    println!(
        "✓ Done with this level. P now has {} nodes, X has {} nodes",
        p.len(),
        x.len()
    );
}
