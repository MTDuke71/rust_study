...
fn parse_mul_instructions(input: &str) -> Result<Vec<(i32, i32)>> {
    ...
    for captures in re.captures_iter(input) {
        ...
        let x = x_str.parse::<i32>()
            .with_context(|| format!("Failed to parse first number: '{}' (full match: '{}')", x_str, captures.get(0).unwrap().as_str()))?;
        let y = y_str.parse::<i32>()
            .with_context(|| format!("Failed to parse second number: '{}' (full match: '{}')", y_str, captures.get(0).unwrap().as_str()))?;
...
mod tests {
    ...
    const PART1_EXAMPLE: &str = "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))";
    const PART2_EXAMPLE: &str = "xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))";
