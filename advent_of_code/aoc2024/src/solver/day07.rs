...
pub fn parse_equations(input: &str) -> Result<Vec<Equation>> {
    ...
    for line in input.lines() {
        ...
        let test_value = parts[0].trim().parse::<u64>()
...
pub fn generate_operator_combinations(positions: usize, include_concat: bool) -> Vec<Vec<Operator>> {
    if positions == 0 {
        return vec![vec![]];
    }
...
292: 11 6 16 20"#;
    ...
    fn test_parse_multiple_equations() {
        ...
        assert_eq!(equations[0], Equation { test_value: 190, numbers: vec![10, 19] });
        assert_eq!(equations[1], Equation { test_value: 3267, numbers: vec![81, 40, 27] });
    }
    ...
    fn test_parse_sample_input() {
        ...
        assert_eq!(equations[0], Equation { test_value: 190, numbers: vec![10, 19] });
        assert_eq!(equations[8], Equation { test_value: 292, numbers: vec![11, 6, 16, 20] });
    }
    ...
    fn test_parse_edge_cases() {
        ...
        assert_eq!(equations[0], Equation { test_value: 42, numbers: vec![42] });
        ...
        assert_eq!(equations[0], Equation { test_value: 100, numbers: vec![10, 20] });
    }
    ...
    fn test_evaluate_edge_cases() {
        ...
        assert_eq!(evaluate_expression(&[1, 2], &[Operator::Add, Operator::Multiply]), 0);
    }
    ...
    fn test_generate_combinations_three_ops() {
        ...
        for combo in expected {
            assert!(combinations.contains(&combo), "Missing combination: {:?}", combo);
        }
    }
    ...
    fn test_validate_solvable_equation() {
        let equation = Equation { test_value: 190, numbers: vec![10, 19] };
    ...
    fn test_validate_unsolvable_equation() {
        let equation = Equation { test_value: 83, numbers: vec![17, 5] };
        assert!(!can_equation_be_solved(&equation, false));
    ...
    fn test_validate_multiple_solutions() {
        let equation = Equation { test_value: 3267, numbers: vec![81, 40, 27] };
        assert!(can_equation_be_solved(&equation, false));
    ...
    fn test_validate_with_concatenation() {
        ...
        let equation1 = Equation { test_value: 156, numbers: vec![15, 6] };
        ...
        let equation2 = Equation { test_value: 7290, numbers: vec![6, 8, 6, 15] };
        ...
        let equation3 = Equation { test_value: 192, numbers: vec![17, 8, 14] };
    ...
    fn test_validate_single_number() {
        let equation1 = Equation { test_value: 42, numbers: vec![42] };
        assert!(can_equation_be_solved(&equation1, false)); // Exact match
        
        let equation2 = Equation { test_value: 42, numbers: vec![43] };
    ...
    fn test_validate_sample_equations() {
        ...
        let solvable_part1: Vec<_> = equations.iter()
            .filter(|eq| can_equation_be_solved(eq, false))
            .collect();
        ...
        let solvable_part2: Vec<_> = equations.iter()
            .filter(|eq| can_equation_be_solved(eq, true))
            .collect();
    ...
    fn test_complex_concatenation_scenarios() {
        ...
        let equation = Equation { test_value: 123456789, numbers: vec![123, 456, 789] };
        // 123 || 456 || 789 = 123456789
        assert!(can_equation_be_solved(&equation, true));
        
        // Mixed operations with concatenation
        let _equation2 = Equation { test_value: 2468, numbers: vec![2, 4, 6, 8] };
