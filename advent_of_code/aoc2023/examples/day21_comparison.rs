...
fn main() {
    let input = std::fs::read_to_string("inputs/day21.txt").expect("Failed to read day21.txt");
    ...
    if result1 != result2 {
        ...
        println!(
            "  Difference:    {}",
            (result1 as i64 - result2 as i64).abs()
        );
    } else {
    ...
    println!(
        "  Speedup: {:.2}× faster (geometric vs extrapolation)",
        speedup
    );
    println!();
