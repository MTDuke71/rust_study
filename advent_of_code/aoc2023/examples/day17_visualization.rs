...
fn find_path_with_tracking(grid: &Grid<u8>, min_straight: u8, max_straight: u8) -> (usize, Vec<Coord>) {
...
fn reconstruct_path(predecessors: &HashMap<State, State>, mut current: State, start: Coord) -> Vec<Coord> {
...
fn visualize_path(grid: &Grid<u8>, path: &[Coord], title: &str) {
    ...
    let path_map: HashMap<Coord, usize> = path.iter()
        .enumerate()
        .map(|(i, &coord)| (coord, i))
        .collect();
    ...
    let total_heat: usize = path.iter().skip(1).map(|&pos| *grid.get(pos).unwrap() as usize).sum();
    println!("Total heat loss: {}", total_heat);
}

fn save_path_as_html(grid: &Grid<u8>, path: &[Coord], filename: &str, title: &str, cost: usize) -> std::io::Result<()> {
    ...
    writeln!(file, ".start {{ background: #4ec9b0; color: #000; font-weight: bold; }}")?;
    writeln!(file, ".end {{ background: #ff6b6b; color: #fff; font-weight: bold; }}")?;
    ...
    writeln!(file, "<p><strong>Path Length:</strong> {} steps</p>", path.len())?;
    writeln!(file, "<p><strong>Grid Size:</strong> {}x{}</p>", grid.width(), grid.height())?;
    writeln!(file, "</div>")?;
