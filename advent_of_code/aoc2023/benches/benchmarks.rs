...
fn benchmark_day19(c: &mut Criterion) {
    ...
    c.bench_function("day19_part1_parse_only", |b| {
        b.iter(|| {
            ...
            // Parse parts
            let parts: Vec<_> = parts_text
                .lines()
                .map(|l| day19::parse_part(black_box(l)))
                .collect();
...
fn benchmark_day20(c: &mut Criterion) {
    ...
    c.bench_function("day20_part1", |b| b.iter(|| day20::part1(black_box(input))));

    c.bench_function("day20_part2", |b| b.iter(|| day20::part2(black_box(input))));
}

fn benchmark_day21(c: &mut Criterion) {
    let input = include_str!("../inputs/day21.txt");

    c.bench_function("day21_part1", |b| b.iter(|| day21::part1(black_box(input))));
...
fn benchmark_day23(c: &mut Criterion) {
    ...
    c.bench_function("day23_part1", |b| {
        b.iter(|| day23::solve_part1(black_box(input)))
    });

    c.bench_function("day23_part2", |b| {
        b.iter(|| day23::solve_part2(black_box(input)))
    });
}

fn benchmark_day24(c: &mut Criterion) {
    let input = include_str!("../inputs/day24.txt");

    c.bench_function("day24_part1", |b| {
        b.iter(|| day24::solve_part1(black_box(input)))
    });

    c.bench_function("day24_part2", |b| {
        b.iter(|| day24::solve_part2(black_box(input)))
    });
}

fn benchmark_day25(c: &mut Criterion) {
    let input = include_str!("../inputs/day25.txt");

    c.bench_function("day25_part1", |b| {
        b.iter(|| day25::solve_part1(black_box(input)))
    });

    c.bench_function("day25_part2", |b| {
        b.iter(|| day25::solve_part2(black_box(input)))
    });
}
