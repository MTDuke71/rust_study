...
fn main() -> Result<()> {
    ...
    let day: usize = args
        .next()
        .ok_or_else(|| anyhow::anyhow!("usage: cargo run -p aoc2023 -- <day> [input_file] [--debug]"))?
        .parse()?;
    ...
    let path = input_file.unwrap_or_else(|| format!("advent_of_code/aoc2023/inputs/day{day:02}.txt"));
