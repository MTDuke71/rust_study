...
fn test_parse_real_aoc_examples() {
    ...
    for command_str in &commands {
        ...
        assert!(
            cmd.x1 <= cmd.x2,
            "x1 should be <= x2 in command: {}",
            command_str
        );
        assert!(
            cmd.y1 <= cmd.y2,
            "y1 should be <= y2 in command: {}",
            command_str
        );
        ...
        assert!(
            cmd.x1 < 1000,
            "x1 should be < 1000 in command: {}",
            command_str
        );
        assert!(
            cmd.y1 < 1000,
            "y1 should be < 1000 in command: {}",
            command_str
        );
        assert!(
            cmd.x2 < 1000,
            "x2 should be < 1000 in command: {}",
            command_str
        );
        assert!(
            cmd.y2 < 1000,
            "y2 should be < 1000 in command: {}",
            command_str
        );
    }
}
