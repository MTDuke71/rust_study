...
fn analyze_reindeer_performance(reindeers: &[Reindeer], duration: u32) -> String {
    ...
    for (i, reindeer) in reindeers.iter().enumerate() {
        ...
        analysis.push_str(&format!(
            "   Cycle: {}s total, {} km per cycle\n",
            cycle_length, distance_per_cycle
        ));
        analysis.push_str(&format!(
            "   Complete cycles: {} ({} km)\n",
            complete_cycles,
            complete_cycles * distance_per_cycle
        ));
...
Dancer can fly 16 km/s for 11 seconds, but then must rest for 162 seconds."#;
        ...
        let expected = calculate_winning_distance(&parse(input).unwrap(), RACE_DURATION);
        assert_eq!(winning_distance, expected);
    }
...
Dancer can fly 16 km/s for 11 seconds, but then must rest for 162 seconds."#;
        ...
        // Calculate expected result manually
        let expected = simulate_race_with_points(&parse(input).unwrap(), RACE_DURATION);
