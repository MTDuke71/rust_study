...
fn main() -> Result<()> {
    ...
    println!("📊 Processing {} lines of light instructions...", input.lines().count());
...
toggle 0,0 through 0,999".to_string()
}
...
fn show_grid_statistics(grid: &Grid<bool>) {
    ...
    println!("   🌑 Lights OFF: {:7} ({:.1}%)", lights_off, 100.0 - percentage_on);
    println!("   📏 Grid Size:  1000x1000 = {} total lights", total_lights);
...
fn analyze_patterns(grid: &Grid<bool>) {
    ...
    let corner_states: Vec<bool> = corners.iter()
        .map(|&coord| grid.get(coord).copied().unwrap_or(false))
        .collect();
    
    println!("   🔲 Corners: TL={} TR={} BL={} BR={}", 
    ...
    // Check center region
    let center_on = (495..=505).flat_map(|y| {
        (495..=505).map(move |x| {
            grid.get(Coord::new(x, y)).copied().unwrap_or(false)
        })
    }).filter(|&light| light).count();
    
    println!("   🎯 Center (495-505, 495-505): {}/121 lights on", center_on);
    
    // Check edges
    let top_edge_on = (0..1000).filter(|&x| {
        grid.get(Coord::new(x, 0)).copied().unwrap_or(false)
    }).count();
    
    let left_edge_on = (0..1000).filter(|&y| {
        grid.get(Coord::new(0, y)).copied().unwrap_or(false)
    }).count();
