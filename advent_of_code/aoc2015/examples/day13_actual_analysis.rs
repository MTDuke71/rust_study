...
fn main() {
    ...
    print_arrangement_table(
        "Part 1 - Original Guest List",
        &graph,
        &part1_arrangement,
        part1_happiness,
    );
    ...
    let change_symbol = if happiness_change >= 0 {
        "📈"
    } else {
        "📉"
    };
    ...
    println!(
        "  🥇 Part 1 (8 people):     {:>6} happiness",
        part1_happiness
    );
    println!(
        "  🥈 Part 2 (9 people):     {:>6} happiness",
        part2_happiness
    );
    println!(
        "  {} Impact of adding you: {:>6} happiness",
        change_symbol, happiness_change
    );
    ...
    if let Some(your_position) = part2_arrangement.iter().position(|p| p == "You") {
        let left_neighbor = &part2_arrangement
            [(your_position + part2_arrangement.len() - 1) % part2_arrangement.len()];
        ...
        println!(
            "  • Position: {} of {} ({})",
            your_position + 1,
            part2_arrangement.len(),
            if your_position < part2_arrangement.len() / 2 {
                "first half"
            } else {
                "second half"
            }
