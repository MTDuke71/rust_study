...
fn main() {
    ...
    println!(
        "  Speedup: {:.1}×",
        original_time.as_nanos() as f64 / rotation_time.as_nanos() as f64
    );
    ...
    println!(
        ...
        factorial(people.len() - 1) / 2
    );
    println!(
        "  Speedup: {:.1}×",
        original_time.as_nanos() as f64 / full_time.as_nanos() as f64
    );
    ...
    println!("===================");
    println!(
        "  All methods found same happiness? {}",
    ...
    println!(
        "  Rotation fix: (n-1)! = {} ({:.1}× reduction)",
        rotation_perms,
        original_perms as f64 / rotation_perms as f64
    );
    println!(
        "  + Reflection: (n-1)!/2 = {} ({:.1}× total reduction)",
        full_perms,
        original_perms as f64 / full_perms as f64
    );
}
...
fn generate_permutations_with_callback<T: Clone, F: FnMut(&[T])>(
    items: &mut [T],
    callback: &mut F,
) {
