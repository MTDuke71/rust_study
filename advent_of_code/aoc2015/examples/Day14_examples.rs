...
fn performance_comparison() {
    ...
    for r in &reindeer {
        ...
        println!("{}: Math={}, Sim={}, Match={}", 
            r.name(), math_result, sim_result, math_result == sim_result);
    }
    println!();
}

/// ASCII-based race visualization showing distance over time
fn create_race_graph(reindeer: &[Reindeer], max_time: u32, graph_width: usize, graph_height: usize) {
...
fn lead_changes_analysis() {
    ...
    for second in 1..=max_time {
        let distances: Vec<(String, u32)> = reindeer
            .iter()
            .map(|r| (r.name().to_string(), calculate_distance_optimized(r, second)))
            .collect();
    ...
    for (time, leader, distance) in lead_history {
        println!("  Second {}: {} takes lead with {} km", time, leader, distance);
    }
    println!();
    ...
    let mut final_distances: Vec<(String, u32)> = reindeer
        .iter()
        .map(|r| (r.name().to_string(), calculate_distance_optimized(r, max_time)))
        .collect();
...
fn actual_problem_analysis() {
    ...
    let time_points = vec![200, 400, 600, 800, 1000, 1200, 1400, 1600, 1800, 2000, 2200, 2503];
    ...
    for (i, &_time) in time_points.iter().enumerate() {
        let max_distance = reindeer_distances.iter().map(|distances| distances[i]).max().unwrap_or(0);
        let winners: Vec<&str> = reindeer.iter()
    ...
    print!("{:<12}", "WINNER");
    for time_idx in 0..time_points.len() {
        let max_points = points_data.iter().map(|points| points[time_idx]).max().unwrap_or(0);
        print!("{:>8}", max_points);
    }
    println!();
    ...
    println!("Part 1 Winner: {} with {} km", distance_winner.0, distance_winner.1);
    println!("Part 2 Winner: {} with {} points", 
        reindeer.iter()
...
fn racing_state_snapshots() {
    ...
    for &time in &snapshot_times {
        ...
        for r in &reindeer {
            ...
            println!("  {}: {} km, {} (cycle position: {}/{})", 
                r.name(), distance, status, cycle_pos, r.cycle_length());
        }
        println!();
    }
}
