...
fn apply_brightness_command_hashmap(
    lights: &mut HashMap<(i32, i32), u32>,
    line: &str,
) -> Result<()> {
    let parts: Vec<&str> = line.split_whitespace().collect();
...
fn performance_comparison() {
    ...
    println!(
        "  📊 Memory efficiency: Only stores {} entries vs 1M grid cells",
        hashmap_count
    );
...
fn hashmap_features_demo() {
    ...
    println!(
        "  HashMap size: {} (vs 1,000,000 for full grid)",
        lights.len()
    );
    ...
    let region_lights: Vec<_> = lights
        .iter()
        .filter(|((x, y), _)| *x >= 100 && *x <= 200 && *y >= 100 && *y <= 200)
        .collect();

    println!(
        "  Lights in region (100,100) to (200,200): {}",
        region_lights.len()
    );
    ...
    if let Some(((x, y), brightness)) = lights.iter().max_by_key(|(_, &brightness)| brightness) {
        println!(
            "  Brightest light: ({}, {}) with brightness {}",
            x, y, brightness
        );
    }
    ...
    println!(
        ...
        lights.len() * (8 + 4 + 24)
    ); // coordinate + brightness + HashMap overhead
}
