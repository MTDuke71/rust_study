...
fn analyze_graph_structure(graph: &HappinessGraph) {
    ...
    println!(
        "  • Edges (E): {} directed relationships",
        graph.edges.len()
    );
    ...
    for person in people.iter().take(3) {
        // Show first 3 to avoid overflow
...
fn analyze_weakest_link_strategy(graph: &HappinessGraph) {
    ...
    if let Some(your_pos) = part2_arrangement.iter().position(|p| p == "You") {
        let left =
            &part2_arrangement[(your_pos + part2_arrangement.len() - 1) % part2_arrangement.len()];
        ...
        println!(
            "  Connection broken: {}-{} (total strength: {})",
            left, right, broken_strength
        );
        println!("  🚨 **This is NOT the weakest connection!**");
        println!(
            "     The algorithm found the globally optimal solution through exhaustive search,"
        );
...
fn demonstrate_symmetry_optimization(graph: &HappinessGraph) {
    ...
    println!(
        "  Speedup: {:.1}×",
        factorial(n) as f64 / factorial(n - 1) as f64
    );
    ...
    println!(
        "  Total speedup: {:.1}×",
        factorial(n) as f64 / theoretical_perms as f64
    );
    ...
    println!("===============================");
    println!(
        "  Original problem: Find optimal circular arrangement of {} people",
        n
    );
    println!("  Naive approach: {} permutations", factorial(n));
    println!(
        "  Rotation symmetry: {} equivalent arrangements per unique solution",
        n
    );
    println!("  Reflection symmetry: 2 equivalent arrangements (clockwise ↔ counter-clockwise)");
    println!(
        "  Combined: {}×2 = {} equivalent arrangements per unique solution",
        n,
        n * 2
    );
    println!(
        "  Optimized search space: {}/{} = {} permutations",
        factorial(n),
        n * 2,
        factorial(n) / (n * 2)
    );
...
fn find_optimal_rotation_fixed(graph: &HappinessGraph) -> (i32, Vec<String>) {
    ...
    let mut others: Vec<String> = people
        .iter()
...
fn demonstrate_symmetry_equivalence(graph: &HappinessGraph) {
    ...
    for i in 1..=2 {
        ...
        assert_eq!(
            base_happiness, rotated_happiness,
            "Rotation should preserve happiness!"
        );
    }
    ...
    assert_eq!(
        base_happiness, reflected_happiness,
        "Reflection should preserve happiness!"
    );
