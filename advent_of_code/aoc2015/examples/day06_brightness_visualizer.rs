...
fn main() -> Result<()> {
    ...
    println!(
        "📊 Processing {} lines of brightness instructions...",
        input.lines().count()
    );
...
toggle 400,400 through 599,599"
        .to_string()
}
...
fn show_brightness_statistics(grid: &Grid<u32>) {
    ...
    println!(
        "   💡 Cells with Light: {} ({:.1}%)",
        non_zero_cells,
        (non_zero_cells as f64 / total_cells as f64) * 100.0
    );
    println!(
        "   🌑 Dark Cells:      {} ({:.1}%)",
        zero_cells,
        (zero_cells as f64 / total_cells as f64) * 100.0
    );
...
fn analyze_brightness_distribution(grid: &Grid<u32>) {
    ...
    for (min_b, max_b, label) in ranges {
        let count = grid
            .iter()
            .filter(|&&b| b >= min_b && (max_b == u32::MAX || b <= max_b))
            .count();
        if count > 0 {
            println!(
                "   {} ({}-{}): {} cells",
    ...
    println!("\n🔍 Sample Locations:");
    println!(
        "   🎯 Center (500,500): brightness {}",
        grid.get(center).copied().unwrap_or(0)
    );
