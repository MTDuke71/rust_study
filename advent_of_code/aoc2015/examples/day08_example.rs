    ...
    pub fn summarize(&self) -> String {
        format!(
            ...
            if self.original_chars > 0 {
                100.0 * (self.original_chars - self.processed_chars) as f64 / self.original_chars as f64
            } else { 0.0 },
            self.quotes_removed,
        ...
        match chars[i] {
            ...
            '\\' if i + 1 < chars.len() => {
                match chars[i + 1] {
                    ...
                    '\\' => { processed_count += 1; i += 2; }  // \\ → \
                    '"' => { processed_count += 1; i += 2; }   // \" → "
                    't' => { processed_count += 1; i += 2; }   // \t → tab
                    'n' => { processed_count += 1; i += 2; }   // \n → newline
                    'r' => { processed_count += 1; i += 2; }   // \r → carriage return
                    '0' => { processed_count += 1; i += 2; }   // \0 → null
                    '\'' => { processed_count += 1; i += 2; }  // \' → '
...
pub fn solve_day8_part1(input: &str) -> (usize, usize, usize) {
    ...
    (total_code, total_memory, total_code.saturating_sub(total_memory))
}
...
fn generate_test_data() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (r#""Mixed: \t\x48\x65\x6C\x6C\x6F \\ end""#, "Complex mixed escapes"),
        (r#""\x48\x65\x6C\x6C\x6F\x20\x57\x6F\x72\x6C\x64""#, "Hello World in hex"),
        (r#""Invalid: \xGG and \z sequences""#, "Invalid escape sequences"),
...
mod tests {
    ...
    fn test_basic_character_counting() {
        ...
        for (input, expected_code, expected_memory) in test_cases {
            let counts = count_processed_characters(input);
            assert_eq!(counts.code_chars, expected_code, 
                      "Code count mismatch for: {}", input);
            assert_eq!(counts.memory_chars, expected_memory,
                      "Memory count mismatch for: {}", input);
        }
    }
