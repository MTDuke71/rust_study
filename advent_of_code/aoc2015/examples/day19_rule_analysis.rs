...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (pattern, element) in &reverse_rules {
        println!("   '{}' → '{}' (length: {})", pattern, element, pattern.len());
    }
    ...
    for (pattern, element) in &to_patterns {
        pattern_conflicts.entry(pattern.clone()).or_insert_with(Vec::new).push(element.clone());
    }
    
    let mut has_conflicts = false;
    for (pattern, elements) in &pattern_conflicts {
        if elements.len() > 1 {
            println!("   ⚠️  CONFLICT: Pattern '{}' maps to multiple elements: {:?}", pattern, elements);
            has_conflicts = true;
        }
    }
    ...
    let bracketed_patterns = reverse_rules.iter()
        .filter(|(pattern, _)| pattern.contains("Rn") && pattern.contains("Ar"))
        .count();
    
    let simple_patterns = reverse_rules.iter()
        .filter(|(pattern, _)| !pattern.contains("Rn") && !pattern.contains("Ar"))
        .count();
