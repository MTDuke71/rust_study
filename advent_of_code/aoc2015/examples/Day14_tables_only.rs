...
fn main() {
    ...
    let points_winner_idx = points_data.iter()
        .enumerate()
    ...
    println!("🏆 Part 2 Winner: {} with {} points", 
        reindeer[points_winner_idx].name(), final_points);
    
    // Show the key insight
    println!("\n💡 Key Insight:");
    if distance_winner.0 != reindeer[points_winner_idx].name() {
        println!("   Different winners! {} wins by distance, {} wins by points.", 
            distance_winner.0, reindeer[points_winner_idx].name());
        println!("   This shows why Part 2's second-by-second scoring creates different results!");
    } else {
        println!("   Same winner for both parts - {} dominates both distance and points!", 
            distance_winner.0);
    }
}