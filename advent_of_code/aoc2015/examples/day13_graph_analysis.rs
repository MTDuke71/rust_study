...
fn main() {
    ...
    let mut all_edges: Vec<((String, String), i32)> = graph
        .edges
        .iter()
    ...
    println!(
        "  🚨 This is the STRONGEST connection! Breaking it would lose {} happiness",
        connection_strength
    );
    ...
    println!(
        "  💡 This connection was broken instead (only {} happiness lost)",
        broken_connection
    );
    ...
    println!(
        "🔸 Current approach: Check all {} permutations",
        factorial(people.len())
    );
    println!(
        "🔸 With rotation optimization: Check {} unique arrangements",
        factorial(people.len()) / people.len()
    );
    println!(
        "🔸 With reflection + rotation: Check {} unique arrangements",
        factorial(people.len()) / (people.len() * 2)
    );
    ...
    println!(
        "  Rotation-optimized:    {:>8} ({:.1}× speedup)",
        rotation_only,
        original_perms as f64 / rotation_only as f64
    );
    println!(
        "  Rotation + Reflection: {:>8} ({:.1}× speedup)",
        both_optimizations,
        original_perms as f64 / both_optimizations as f64
    );
...
fn demonstrate_symmetry_optimization(graph: &HappinessGraph) {
    ...
    println!(
        "  Original:  {:?} → happiness: {}",
        base_arrangement, original_happiness
    );
    println!(
        "  Reflected: {:?} → happiness: {}",
        reflected, reflected_happiness
    );
