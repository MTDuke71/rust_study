...
| **Total Runtime** | 420.90ms |
| **Average per Day** | 16.84ms |
...
| [25](days/day25_function_guide.md) | — | 🎄 | 37.53ms | Turing machine simulation over a `Vec<u8>` tape with `isize` offset | None | 12.9M-step trace at ~2.9 ns/step; flat `Vec<[Rule; 2]>` lookup table indexed by `(state, value)`; tape grows by doubling and re-centering when the cursor escapes the buffer. No Part 2 puzzle — Day 25's second star is the celebration freebie for finishing all of 1–24 |
...
- [Day 25](days/day25_function_guide.md) - The Halting Problem | [Code](../src/solver/day25.rs) ✅ ⭐
...
| 25 | Turing-machine simulation with `Vec<u8>` tape + `isize` offset + flat `[Rule; 2]` lookup | The tape is conceptually infinite, so wrap a `Vec<u8>` with a logical-to-physical offset: position `pos` lives at buffer index `pos + offset`. When the cursor escapes the buffer, double its size and copy the contents into the middle, then shift `offset` so logical positions stay stable — amortised O(1) growth, never triggered on this input because 64 KiB starter is plenty for the cursor's actual excursion. Rules live in `Vec<[Rule; 2]>` so `(state, value)` lookup is two contiguous loads from L1; with `Rule = (u8, i8, u8)` the entire 12-rule table is < 50 bytes. Hot loop is ~6 ops/step (compute idx, bounds-check, load, lookup, store, update); 12.9M steps in 37.5 ms is ~2.9 ns/step, L1-bound. Day 25 has no Part 2 puzzle — the second star unlocks once Days 1–24 are complete |
