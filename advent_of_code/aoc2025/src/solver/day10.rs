        ...
        if let Some(pivot_row) = (row..num_lights).find(|&r| matrix[r][col] == 1) {
            ...
            for (other_row_idx, other_row) in matrix.iter_mut().enumerate() {
                if other_row_idx != row && other_row[col] == 1 {
                    for (c, &pivot_value) in pivot_row_values.iter().enumerate().take(num_buttons + 1) {
...
fn solve_machine_joltage(machine: &Machine, targets: &[i64]) -> usize {
    ...
    let raw_buttons: Vec<Vec<usize>> = machine.buttons
        .iter()
    ...
    let mut problem = vars.minimise(button_presses.iter().sum::<Expression>()).using(default_solver);
    ...
    match problem.solve() {
        Ok(solution) => {
            ...
            if !is_integer {
                eprintln!("WARNING: Non-integer solution detected! Values: {:?}", values);
...
pub fn solve_part2(input: &str) -> Result<String> {
    ...
    for line in input.lines() {
        ...
        eprintln!("Processing machine {}: {} counters, {} buttons, targets: {:?}", 
                  machine_num, joltages.len(), machine.num_buttons(), joltages);
        let presses = solve_machine_joltage(&machine, &joltages);
        eprintln!("  -> {} presses (total so far: {})", presses, total_presses + presses);
        total_presses += presses;
    }
