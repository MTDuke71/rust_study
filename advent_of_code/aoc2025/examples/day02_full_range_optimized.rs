    ...
    for total_digits in min_digits..=max_digits {
        ...
        for pattern_len in 1..=total_digits / 2 {
            ...
            let pattern_start = if pattern_len == 1 {
                ...
                10_u64.pow((pattern_len - 1) as u32)
            };
...
fn main() {
    ...
    println!(
        "Part 1: {} (count: {}) in {:.2?}",
        sum_part1, count_part1, elapsed_part1
    );
    println!(
        "Part 2: {} (count: {}) in {:.2?}",
        sum_part2, count_part2, elapsed_part2
    );
    ...
    for digits in 1..=10 {
        let divisors: Vec<usize> = (1..=digits / 2).filter(|&p| digits % p == 0).collect();
