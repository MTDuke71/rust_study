...
fn test_line_graph_from_file() {
    let content = fs::read_to_string("tests/data/line_graph.txt")
...
fn test_star_graph_from_file() {
    let content = fs::read_to_string("tests/data/star_graph.txt")
        .expect("Should read test data file");
...
fn test_binary_tree_from_file() {
    let content = fs::read_to_string("tests/data/binary_tree.txt")
        .expect("Should read test data file");
...
fn test_edge_cases() {
    ...
    let (diameter, path) = diameter_finder.find_diameter().expect("Should handle two nodes");
...
fn parse_tree_file(content: &str) -> (usize, Vec<(usize, usize)>) {
    ...
    let n: usize = lines[0].parse().expect("First line should be number of nodes");
...
fn verify_path_contiguity(_diameter_finder: &TreeDiameter, path: &[usize]) {
    for window in path.windows(2) {
        ...
        assert!(curr != next, "Path should not have duplicate consecutive nodes");
    }
}
