...
impl TextAnalysisEngine {
    ...
    pub fn add_document(&mut self, doc_id: String, content: String) {
        ...
        // Store content
        self.document_content.insert(doc_id.clone(), content.clone());
        ...
        for word in &words {
            *self.word_frequencies.entry(word.clone()).or_insert(0) += 1;
            self.document_index.entry(word.clone()).or_insert_with(HashSet::new).insert(doc_id.clone());
        }
    ...
    pub fn analyze_document(&mut self, doc_id: &str) -> Option<AnalysisResult> {
        ...
        // Cache the result
        self.analysis_cache.insert(doc_id.to_string(), result.clone());
    ...
    pub fn find_similar_documents(&self, doc_id: &str, threshold: f32) -> Vec<(String, f32)> {
        ...
        for (other_doc_id, other_content) in &self.document_content {
            ...
            let other_words: HashSet<String> = other_content.to_lowercase()
                .split_whitespace()
                .map(|word| word.trim_matches(|c: char| !c.is_alphabetic()))
                .filter(|word| !word.is_empty())
                .map(|word| word.to_string())
    ...
    pub fn get_statistics(&self) -> SystemStats {
        SystemStats {
            ...
            top_words: {
                ...
                words.into_iter().take(10).map(|(word, count)| (word.clone(), *count)).collect()
            },
        }
    }
}
...
impl GameWorld {
    ...
    pub fn find_entities_in_radius(&self, center: (i32, i32), radius: i32) -> Vec<(String, (i32, i32))> {
        self.entities.iter()
            .filter(|(_, &(x, y))| {
    ...
    pub fn get_exploration_stats(&self) -> (usize, f32) {
        let total_accessible = self.grid.iter()
...
fn comprehensive_demo() {
    ...
    engine.add_document("doc1".to_string(), 
        "Rust is a systems programming language focused on safety and performance".to_string());
    engine.add_document("doc2".to_string(), 
        "Python is a high-level programming language great for rapid development".to_string());
    engine.add_document("doc3".to_string(), 
        "JavaScript is a versatile programming language used for web development".to_string());
    engine.add_document("doc4".to_string(), 
        "Rust offers memory safety without garbage collection and excellent performance".to_string());
    
    // Perform analyses (demonstrates caching)
    println!("\n🔍 Document Analysis:");
    for doc_id in ["doc1", "doc2", "doc1", "doc3", "doc1"] { // Note: doc1 repeated
        if let Some(analysis) = engine.analyze_document(doc_id) {
            println!("  {} -> {} words, {} unique, readability: {:.1}", 
    ...
    println!("  Documents containing 'programming' AND 'language': {:?}", search_results);
...
fn final_challenges() {
    ...
    println!("💡 Remember: HashMap for key-value, HashSet for uniqueness, caching for performance!");
}
