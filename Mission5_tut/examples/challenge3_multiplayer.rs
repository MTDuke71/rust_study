...
pub enum GameEventType {
    PlayerJoined { room_id: RoomId },
    PlayerLeft { room_id: RoomId },
    PlayerMoved { from: Position, to: Position },
    PlayerProximity { other_player: PlayerId, distance: f32 },
...
impl MultiplayerGameManager {
    ...
    pub fn add_room(&mut self, room: Room) {
        ...
        self.room_memberships.insert(room.id.clone(), HashSet::new());
    ...
    pub fn add_player(&mut self, player: Player) -> Result<(), String> {
        ...
        self.position_grid.entry(grid_pos).or_insert_with(HashSet::new).insert(player_id.clone());
    ...
    pub fn move_player(&mut self, player_id: &PlayerId, new_position: Position) -> Result<(), String> {
        let old_position = self.player_positions.get(player_id)
            .ok_or_else(|| format!("Player {} not found", player_id))?
            .clone();
        
        // Update position
        self.player_positions.insert(player_id.clone(), new_position);
        ...
        if old_grid_pos != new_grid_pos {
            ...
            self.position_grid.entry(new_grid_pos).or_insert_with(HashSet::new).insert(player_id.clone());
        }
        ...
        self.add_event(GameEvent {
            event_type: GameEventType::PlayerMoved { from: old_position, to: new_position },
            player_id: player_id.clone(),
            timestamp: Instant::now(),
        });
        
        // Clear proximity cache for this player (positions changed)
        self.proximity_cache.retain(|(p1, p2), _| p1 != player_id && p2 != player_id);
        ...
        println!("🚶 Player {} moved from {:?} to {:?}", player_id, old_position, new_position);
        
        Ok(())
    }
    
    /// Assign a player to a specific room
    pub fn assign_player_to_room(&mut self, player_id: PlayerId, room_id: RoomId) -> Result<(), String> {
        ...
        if current_members >= room.capacity {
            self.add_event(GameEvent {
                event_type: GameEventType::RoomFull { room_id: room_id.clone() },
                player_id: player_id.clone(),
                timestamp: Instant::now(),
            });
        ...
        self.room_memberships.get_mut(&room_id).unwrap().insert(player_id.clone());
        self.player_room_lookup.insert(player_id.clone(), room_id.clone());
        ...
        self.add_event(GameEvent {
            event_type: GameEventType::PlayerJoined { room_id: room_id.clone() },
            player_id: player_id.clone(),
            timestamp: Instant::now(),
        });
    ...
    fn remove_player_from_room(&mut self, player_id: &PlayerId, room_id: &RoomId) {
        ...
        self.add_event(GameEvent {
            event_type: GameEventType::PlayerLeft { room_id: room_id.clone() },
            player_id: player_id.clone(),
            timestamp: Instant::now(),
        });
    ...
    pub fn find_players_in_radius(&mut self, center: Position, radius: f32) -> Vec<(PlayerId, Position, f32)> {
    ...
    pub fn find_path_between_rooms(&mut self, from_room: &RoomId, to_room: &RoomId) -> Option<Vec<RoomId>> {
        ...
        println!("🔄 Cache MISS: Computing path from {} to {}", from_room, to_room);
    ...
    pub fn get_room_stats(&self) -> Vec<(RoomId, usize, usize)> {
        self.rooms.iter().map(|(room_id, room)| {
            let current_occupancy = self.room_memberships.get(room_id)
    ...
    fn check_player_proximity(&mut self, player_id: &PlayerId) {
        ...
        for (other_player_id, _other_pos, distance) in nearby_players {
            if other_player_id != *player_id {
                ...
                let should_notify = if let Some((cached_distance, cached_time)) = self.proximity_cache.get(&cache_key) {
                    // Only notify if distance changed significantly or enough time passed
                    (distance - cached_distance).abs() > 5.0 || cached_time.elapsed() > Duration::from_secs(2)
                } else {
                    true
                };
                
                if should_notify {
                    self.proximity_cache.insert(cache_key, (distance, Instant::now()));
                    ...
                    println!("👥 Proximity: {} and {} are {:.1} units apart", 
    ...
    pub fn get_game_statistics(&self) -> GameStatistics {
        ...
        GameStatistics {
            ...
            active_players: self.players.values().filter(|p| p.current_room.is_some()).count(),
            cache_hit_ratio,
    ...
    fn find_room_for_position(&self, position: Position) -> Option<RoomId> {
        for (room_id, room) in &self.rooms {
            ...
            if position.x >= tl.x && position.x <= br.x &&
...
fn multiplayer_demo() {
    ...
    for _ in 0..3 {
        if let Some(path) = game.find_path_between_rooms(&"lobby".to_string(), &"arena1".to_string()) {
...
mod tests {
    ...
    fn test_player_creation_and_movement() {
        ...
        // Add a player
        let player = Player::new("test_player".to_string(), "Test Player".to_string(), Position::new(50, 50));
        assert!(game.add_player(player).is_ok());
        
        // Move player
        assert!(game.move_player(&"test_player".to_string(), Position::new(60, 60)).is_ok());
        
        // Check position was updated
        assert_eq!(game.player_positions.get("test_player"), Some(&Position::new(60, 60)));
    }
    ...
    fn test_room_assignment_and_capacity() {
        ...
        let player1 = Player::new("player1".to_string(), "Player 1".to_string(), Position::new(25, 25));
        assert!(game.add_player(player1).is_ok());
        
        // Add second player outside room bounds to avoid auto-assignment
        let player2 = Player::new("player2".to_string(), "Player 2".to_string(), Position::new(75, 75));
        assert!(game.add_player(player2).is_ok());
        
        // Try to manually assign second player to full room - should fail
        assert!(game.assign_player_to_room("player2".to_string(), "small_room".to_string()).is_err());
    }
    ...
    fn test_proximity_detection() {
        ...
        let player1 = Player::new("player1".to_string(), "Player 1".to_string(), Position::new(0, 0));
        let player2 = Player::new("player2".to_string(), "Player 2".to_string(), Position::new(10, 0)); // 10 units away
        let player3 = Player::new("player3".to_string(), "Player 3".to_string(), Position::new(100, 0)); // 100 units away
