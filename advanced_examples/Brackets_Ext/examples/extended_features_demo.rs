...
fn demo_configurable_alphabet() {
    ...
    match validate_with_options("<test>", &default_opts) {
        ...
        Err(_) => {
            println!("❌ '<test>' is invalid with default alphabet (angle brackets not supported)")
        }
    }
            ...
            for (i, err) in errs.iter().enumerate() {
                match &err.kind {
                    BracketErrorKind::UnclosedOpenings {
                        ...
                        println!(
                            "  {}. Position {}: Expected '{}' to close bracket at position {}",
                            i + 1,
                            err.index,
                            expected,
                            open_index
...
mod demo_tests {
    ...
    fn test_error_collection_comprehensive() {
        ...
        assert!(errors
            .iter()
            .any(|e| matches!(e.kind, BracketErrorKind::UnclosedOpenings { .. })));
    }
    ...
    fn test_unclosed_policy_difference() {
        ...
        match (&latest_err[0].kind, &earliest_err[0].kind) {
            (
                BracketErrorKind::UnclosedOpenings {
                    open_index: latest_pos,
                    ..
                },
                BracketErrorKind::UnclosedOpenings {
                    open_index: earliest_pos,
                    ..
                },
            ) => {
