...
fn test_small_maze_from_file() {
    let content =
        fs::read_to_string("tests/data/maze_small.txt").expect("Should read test data file");
...
fn test_medium_maze_from_file() {
    let content =
        fs::read_to_string("tests/data/maze_medium.txt").expect("Should read test data file");
...
fn test_no_solution_maze_from_file() {
    let content =
        fs::read_to_string("tests/data/maze_no_solution.txt").expect("Should read test data file");
...
fn test_queue_capacity_analysis() {
    ...
    for (file, expected_capacity) in test_cases {
        let content = fs::read_to_string(file).expect("Should read test data file");
...
fn test_optimal_path_guarantee() {
    ...
    assert!(
        is_top_path || is_bottom_path,
        "Should choose one of the two optimal paths, but got: {:?}",
        path
    );
}
...
fn test_maze_format_validation() {
    ...
    // Missing start
    let invalid_maze1 = vec!["...".to_string(), "...".to_string(), "..G".to_string()];
    assert!(MazeSolver::new(invalid_maze1).is_none());

    // Missing goal
    let invalid_maze2 = vec!["S..".to_string(), "...".to_string(), "...".to_string()];
    ...
    // Multiple starts
    let invalid_maze4 = vec!["S.S".to_string(), "...".to_string(), "..G".to_string()];
