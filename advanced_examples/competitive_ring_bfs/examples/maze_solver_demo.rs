...
fn demo_cache_efficiency() {
    ...
    println!(
        "🔄 RingBufferQueue capacity: {} (exactly grid size)",
...
fn demo_flood_fill() {
    ...
    println!(
        "   Queue capacity: {} (exactly what's needed)",
        solver.required_queue_capacity()
    );
...
fn demo_position_coordinate_analysis() {
    ...
    println!(
        "📏 Manhattan distance: {}",
        (goal.row as i32 - start.row as i32).abs() + (goal.col as i32 - start.col as i32).abs()
    );
    ...
    println!(
        "   Position::new(2, 3) = ({}, {})",
        example_pos.row, example_pos.col
    );

    let center = Position::new(rows / 2, cols / 2);
    println!(
        "   Grid center ≈ Position::new({}, {}) = ({}, {})",
        rows / 2,
        cols / 2,
        center.row,
        center.col
    ...
    if let Some(path) = solver.find_shortest_path() {
        ...
        println!(
            "   Path efficiency: {:.1}% (lower is better due to obstacles)",
...
fn simulate_multi_source_bfs(maze: &[String]) {
    ...
    for &hospital in &hospitals {
        queue
            .enqueue((hospital, 0))
            .expect("Queue has sufficient capacity");
    ...
    while let Some((current_pos, current_dist)) = queue.dequeue() {
        ...
        for neighbor in current_pos.neighbors(rows, cols) {
            ...
            queue
                .enqueue((neighbor, new_distance))
                .expect("Queue capacity sufficient");
    ...
    println!(
        "     RingBufferQueue peak usage: ~{} entries",
        distances.len()
    );
