...
fn exercise1_benchmark_shortest_path() {
    ...
    println!(
        "🎯 Goal: Measure shortest_path performance across different graph sizes and structures\n"
    );
    ...
    println!(
        "│ 100 nodes   │ {:>9.2}μs │    100%     │      ~8      │",
        small_time
    );
    println!(
        "│ 1000 nodes  │ {:>9.2}μs │    100%     │     ~17      │",
        medium_time
    );
    println!(
        "│ 5000 nodes  │ {:>9.2}μs │    100%     │     ~35      │",
        large_time
    );
    println!("└─────────────┴─────────────┴─────────────┴──────────────┘\n");
    ...
    println!(
        "  • 10x size increase → {:.1}x time increase",
        scaling_factor_medium
    );
    println!(
        "  • 50x size increase → {:.1}x time increase",
        scaling_factor_large
    );
    ...
    println!(
        "  • Dense graph (20 edges/node): {:>9.2}μs ({:.1}x slower)",
        dense_time,
        dense_time / medium_time
    );
    println!(
        "  • Linear chain:                {:>9.2}μs ({:.1}x faster)",
        linear_time,
        medium_time / linear_time
    );
...
fn exercise2_compare_visited_tracking() {
    ...
    for size in test_sizes {
        ...
        println!(
            "│ {:>9} │ {:>9.1}μs │ {:>9.1}μs │    {:>6.2}x │ {:>4}KB/{:>3}KB │",
            size,
            hashset_time,
            vec_time,
            ratio,
...
fn exercise4_optimize_cycle_detection() {
    ...
    println!(
        "│ Basic DFS            │ {:>9.1}μs │ {:>9.1}μs │      No      │",
        basic_acyclic_time, basic_cyclic_time
    );
    println!(
        "│ Optimized DFS        │ {:>9.1}μs │ {:>9.1}μs │      No      │",
        optimized_acyclic_time, optimized_cyclic_time
    );
    println!(
        "│ Early Termination    │ {:>9.1}μs │ {:>9.1}μs │     Yes      │",
        early_term_acyclic_time, early_term_cyclic_time
    );
    println!("└──────────────────────┴─────────────┴─────────────┴──────────────┘\n");
    ...
    let basic_improvement =
        (basic_acyclic_time - optimized_acyclic_time) / basic_acyclic_time * 100.0;
    let early_term_improvement =
        (basic_cyclic_time - early_term_cyclic_time) / basic_cyclic_time * 100.0;

    println!("  📈 Performance Improvements:");
    println!(
        "    • Optimized DFS: {:.1}% faster on acyclic graphs",
        basic_improvement
    );
    println!(
        "    • Early termination: {:.1}% faster on cyclic graphs",
        early_term_improvement
    );
...
fn create_linear_graph(nodes: u32) -> HashMap<u32, Vec<u32>> {
    ...
    for i in 0..nodes {
        let neighbors = if i < nodes - 1 { vec![i + 1] } else { vec![] };
        graph.insert(i, neighbors);
    }
...
fn basic_has_cycle(graph: &HashMap<u32, Vec<u32>>) -> bool {
    ...
    for &start in graph.keys() {
        if !visited.contains(&start) && basic_dfs_cycle(graph, start, &mut visited, &mut rec_stack)
        {
            return true;
        }
    }
...
fn early_termination_dfs_cycle(
    graph: &HashMap<u32, Vec<u32>>,
    node: u32,
    state: &mut Vec<NodeState>,
) -> bool {
