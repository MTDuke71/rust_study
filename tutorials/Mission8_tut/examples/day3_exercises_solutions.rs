        ...
        match color.get(&neighbor).unwrap_or(&NodeColor::White) {
            NodeColor::Gray => {
                ...
                if let Some(pos) = path.iter().position(|&n| n == neighbor) {
                    ...
                    if cycle.len() > 1 { // Avoid self-loops as trivial cycles
    ...
    N: Copy + Eq + std::hash::Hash + Debug,
{
    ...
    while !queue_start.is_empty() && !queue_end.is_empty() {
        ...
        if let Some(path) = expand_bidirectional_side(
            graph, &mut queue_start, &mut visited_start, &visited_end,
        ) {
            return Some(path);
        }
        ...
        if let Some(path) = expand_bidirectional_side(
            graph, &mut queue_end, &mut visited_end, &visited_start,
        ) {
            return Some(path);
        }
    }
    ...
    N: Copy + Eq + std::hash::Hash + Debug,
{
    ...
    if visited_other.contains_key(&current) {
        return reconstruct_bidirectional_path(
            current, visited_this, visited_other,
        );
    }
    ...
    N: Copy + Eq + std::hash::Hash + Debug,
{
    ...
    *time += 1;
    ...
    for &neighbor in graph.get(&node).unwrap_or(&Vec::new()) {
        ...
        if !visited.contains(&neighbor) {
            find_bridges_dfs(
                graph, neighbor, node, visited,
                discovery_time, low_time, time, bridges,
            );
...
fn main() {
    ...
    if let Some(bidirectional_path) = bidirectional_bfs(&dag_graph, 0, 5) {
        println!("Bidirectional BFS path from 0 to 5: {:?}", bidirectional_path);
