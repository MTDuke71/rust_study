...
fn shortest_path<T, G>(graph: &G, start: T, end: T) -> Option<Vec<T>>
where
    ...
    if let (Some(start), Some(end)) = (large_maze.start(), large_maze.end()) {
        ...
        match shortest_path(&large_maze, start, end) {
            Some(path) => {
                ...
                println!(
                    "Path efficiency: {:.1}% of theoretical minimum",
                    (path.len() as f64
                        / ((end.0.abs_diff(start.0) + end.1.abs_diff(start.1)) as f64 + 1.0))
                        * 100.0
                );
