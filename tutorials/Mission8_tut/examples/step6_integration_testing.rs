...
mod integration_tests {
    ...
    fn test_large_graph_performance() {
        ...
        assert!(
            bfs_duration.as_millis() < 100,
            "BFS took {}ms, should be under 100ms",
            bfs_duration.as_millis()
        );
        assert!(
            dfs_duration.as_millis() < 100,
            "DFS took {}ms, should be under 100ms",
...
mod property_tests {
    ...
    fn property_dfs_visits_all_reachable() {
        ...
        for graph in graphs {
            for start_node in graph.nodes() {
                ...
                assert_eq!(
                    dfs_set, bfs_set,
                    "DFS and BFS should visit same nodes from start {}",
                    start_node
                );
            }
        }
    ...
    fn property_parent_relationships_valid() {
        ...
        for (child, parent) in &bfs_result.parent {
            ...
            assert!(
                neighbors.contains(child),
                "Parent {} should have child {} as neighbor in BFS",
                parent,
                child
            );
        }
        ...
        for (child, parent) in &dfs_result.parent {
            ...
            assert!(
                neighbors.contains(child),
                "Parent {} should have child {} as neighbor in DFS",
                parent,
                child
            );
        }
    ...
    fn cross_validate_bfs_implementations() {
        ...
        for graph in test_graphs {
            for start_node in 0..graph.node_count.min(5) {
                ...
                assert_eq!(
                    our_set, ref_set,
                    "Our BFS and reference BFS should visit same nodes from {}",
                    start_node
                );
            }
        }
...
fn main() {
    ...
    println!(
        "   BFS visited {} nodes in {:?}",
        large_bfs.visited.len(),
        duration
    );
    ...
    let all_neighbors_discovered_early = start_neighbors.iter().all(|&neighbor| {
        bfs_complete.discovery_time[&neighbor] <= start_discovery_time + start_neighbors.len()
    });
