...
impl<T: fmt::Display> fmt::Display for StackError<T> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            StackError::CapacityExceeded { current_size, max_capacity, .. } => {
                write!(f, "Stack capacity exceeded: {}/{} items", current_size, max_capacity)
            }
...
fn demonstrate_performance_analysis() {
    ...
    println!("   Small stack (1K ops): {:?}, Allocations: {}", 
             small_duration, small_stack.stats().total_allocations);
    ...
    println!("   Large stack (100K ops): {:?}, Allocations: {}", 
             large_duration, large_stack.stats().total_allocations);
    ...
    println!("   Memory usage - Length: {}, Capacity: {}, Efficiency: {:.1}%",
             vec_based.len(), 
             vec_based.capacity(),
...
fn verify_mission1_requirements() {
    ...
    println!("   ✅ REQ-P1: Performance - 10K pushes: {:?}, 10K pops: {:?}", 
             push_time, pop_time);
    ...
    println!("   ✅ REQ-P2: Memory efficiency: {:.1}% (50/100 capacity used)", efficiency);
...
fn demonstrate_error_handling() {
    ...
    match limited_stack.push("overflow".to_string()) {
        ...
        Err(StackError::CapacityExceeded { item, current_size, max_capacity }) => {
            println!("   ❌ Capacity exceeded: tried to push '{}' with {}/{} items", 
...
fn demonstrate_calculator() {
    ...
    fn evaluate_postfix(expr: &str) -> Result<f64, CalcError> {
        ...
        for token in expr.split_whitespace() {
            match token {
                "+" => {
                    let b = stack.pop().map_err(|_| CalcError::StackError("Missing operand".to_string()))?;
                    let a = stack.pop().map_err(|_| CalcError::StackError("Missing operand".to_string()))?;
                    stack.push(a + b).map_err(|e| CalcError::StackError(e.to_string()))?;
                }
                "-" => {
                    let b = stack.pop().map_err(|_| CalcError::StackError("Missing operand".to_string()))?;
                    let a = stack.pop().map_err(|_| CalcError::StackError("Missing operand".to_string()))?;
                ...
                num_str => {
                    ...
                    stack.push(num).map_err(|e| CalcError::StackError(e.to_string()))?;
                }
            }
        }
        
        if stack.len() == 1 {
            stack.pop().map_err(|_| CalcError::StackError("Empty result".to_string()))
        } else {
            Err(CalcError::StackError(format!("Invalid expression: {} values remaining", stack.len())))
        }
    }
...
fn graduation_message() {
    ...
    println!("💻 {} lines of educational code written", count_tutorial_lines());
