...
fn explain_vec_internals() {
    ...
    println!("   Initial - len: {}, cap: {}", stack.len(), stack.capacity());
    ...
    for i in 1..=10 {
        ...
        if new_cap != old_cap {
            println!("   After push({}): len: {}, cap: {} (REALLOCATION!)", 
                     i, stack.len(), stack.capacity());
        }
    }
...
fn analyze_growth_pattern() {
    ...
    for i in 1..=20 {
        ...
        if new_cap > old_cap {
            ...
            println!("   Reallocation #{}: {} → {} (copied {} elements)", 
                     reallocations, old_cap, new_cap, stack.len() - 1);
        }
    }
    ...
    println!("   • Average copies per push: {:.2}", total_copies as f64 / 20.0);
...
fn measure_stack_performance() {
    ...
    println!("   Speedup factor: {:.2}x", duration1.as_nanos() as f64 / duration2.as_nanos() as f64);
...
fn demonstrate_memory_overhead() {
    ...
    println!("   • Overhead per element: {:.2} bytes", vec_overhead as f64 / 1000.0);
    ...
    println!("   • Per-node overhead: {} bytes (includes pointer)", node_size);
    println!("   • Total for 1000 nodes: {} bytes", list_total);
    println!("   • Overhead per element: {:.2} bytes", (node_size - 4) as f64);
...
fn demonstrate_optimizations() {
    ...
    println!("   After 100 pushes - len: {}, cap: {}", stack.len(), stack.capacity());
    
    stack.shrink_to_fit();
    println!("   After shrink_to_fit - len: {}, cap: {}", stack.len(), stack.capacity());
