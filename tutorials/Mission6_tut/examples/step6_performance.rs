...
fn main() {
    ...
    println!("Testing access patterns on {}x{} grid:", test_size, test_size);
    ...
    println!("Row-major (cache-friendly): {:?} (median of {})", row_major_time, iterations);
    println!("Column-major (strided access): {:?} (median of {})", col_major_time, iterations);
    println!("Random access (cache-hostile): {:?} (median of {})", random_time, iterations);
    ...
    println!("Random access vs row-major: {:.2}x slower", row_vs_random_ratio);
    println!("\n💡 Cache Impact Analysis:");
    println!("   • Row-major: Sequential access = optimal cache utilization");
    println!("   • Column-major: Strided access = cache line misses every {}x{} elements", 
             test_size, std::mem::size_of::<i32>());
    ...
    println!("  Vec<Vec<T>>: {:?} (sum: {}) - pointer chase overhead", vec_vec_access, sum_vec_vec);
    println!("  Vec<T> flat: {:?} (sum: {}) - direct calculation", flat_access, sum_flat);
    ...
    println!("  Direct indexing:  {:?} ({:.2}x)", index_median, 
             coord_median.as_secs_f64() / index_median.as_secs_f64());
    println!("  Range-based:      {:?} ({:.2}x)", range_median,
