...
fn main() {
    ...
    let obstacles = vec![
        // Table
        TutorialCoord::new(4, 3), TutorialCoord::new(5, 3), TutorialCoord::new(6, 3),
        TutorialCoord::new(4, 4), TutorialCoord::new(5, 4), TutorialCoord::new(6, 4),
        // Couch
        TutorialCoord::new(9, 2), TutorialCoord::new(10, 2), TutorialCoord::new(11, 2),
        TutorialCoord::new(9, 3), TutorialCoord::new(10, 3), TutorialCoord::new(11, 3),
        // Bookshelf
        TutorialCoord::new(2, 7), TutorialCoord::new(3, 7), TutorialCoord::new(4, 7),
        TutorialCoord::new(5, 7), TutorialCoord::new(6, 7),
    ];
    ...
    if let Some(path) = simple_bfs(&robot_world, robot_start, robot_goal) {
        ...
        for (i, &coord) in path.iter().enumerate() {
            if i > 0 && i < path.len() - 1 { // Skip start and goal
                path_grid.set(coord, '*');
            }
        }
        println!("{}", path_grid);
    ...
    println!("  Temperature range: {:.1}°C - {:.1}°C", stats.min, stats.max);
    ...
    #[allow(dead_code)]
    enum GridError {
        OutOfBounds { coord: TutorialCoord, width: usize, height: usize },
    ...
    impl fmt::Display for GridError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                GridError::OutOfBounds { coord, width, height } => {
                    write!(f, "Coordinate {} is out of bounds for {}x{} grid", coord, width, height)
                }
    ...
    fn safe_grid_operation(grid: &TutorialGrid<i32>, coord: TutorialCoord) -> Result<i32, GridError> {
    ...
    println!("  Grid utilization: {:.1}%", ecosystem.utilization() * 100.0);
...
impl SnakeGame {
    ...
    fn make_move(&mut self, direction: Direction) -> bool {
        ...
        let new_head = match direction {
            ...
            Direction::Down if head.y < self.grid.height() - 1 => TutorialCoord::new(head.x, head.y + 1),
            Direction::Left if head.x > 0 => TutorialCoord::new(head.x - 1, head.y),
            Direction::Right if head.x < self.grid.width() - 1 => TutorialCoord::new(head.x + 1, head.y),
...
fn calculate_temperature_stats(grid: &TutorialGrid<f32>) -> TemperatureStats {
    ...
    TemperatureStats {
        ...
        min: if min_temp != f32::INFINITY { min_temp } else { 0.0 },
        max: if max_temp != f32::NEG_INFINITY { max_temp } else { 0.0 },
        hot_spots,
    }
}

fn simple_bfs(grid: &TutorialGrid<char>, start: TutorialCoord, goal: TutorialCoord) -> Option<Vec<TutorialCoord>> {
    ...
    fn simulate_turn(&mut self) {
        ...
        for old_coord in creature_coords {
            if let Some(creature_type) = self.creatures.remove(&old_coord) {
                ...
                let new_coord = directions.into_iter()
                    .find(|coord| self.grid.in_bounds(*coord) && !self.creatures.contains_key(coord))
                    .unwrap_or(old_coord);
