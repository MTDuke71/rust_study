...
fn demonstrate_trait_implementations() {
    ...
    println!("      After modification: queue1 == queue2: {}", queue1 == queue2);
...
fn demonstrate_performance_optimizations() {
    ...
    println!("      Normal capacity ({}): uses modulo operation", capacity_normal);
    for i in 0..5 {
        let next = (i + 1) % capacity_normal;
        println!("      {} -> {} (using % operator)", i, next);
    }
    
    println!("\n      Power-of-2 capacity ({}): uses bitwise AND", capacity_power2);
    for i in 0..5 {
        let next_mod = (i + 1) % capacity_power2;
        let next_and = (i + 1) & (capacity_power2 - 1);
        println!("      {} -> {} (% gives {}, & gives {})", i, next_and, next_mod, next_and);
    }
