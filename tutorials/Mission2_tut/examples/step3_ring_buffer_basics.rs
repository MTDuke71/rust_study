...
fn demonstrate_modulo_arithmetic() {
    ...
    let mut current = 0;
    for step in 0..10 {
        println!("      Step {}: index {} → next index {}", 
                step, current, (current + 1) % capacity);
    ...
    for step in 0..8 {
        ...
        println!("      Step {}: index {} → prev index {}", step, current, prev);
        current = prev;
    }
...
fn demonstrate_wraparound() {
    ...
    impl<T: Clone + std::fmt::Display> RingBufferVisualizer<T> {
        ...
        fn show_state(&self, operation: &str) {
            ...
            let display: Vec<String> = self.buffer.iter()
                .map(|opt| match opt {
                    Some(val) => format!("{}", val),
                    None => "_".to_string(),
                })
            ...
            println!("      H={}, T={}, Len={}/{}", 
                    self.head, self.tail, self.length, self.capacity());
        }
    }
