...
fn demonstrate_option_memory_layout() {
    ...
    println!("      Size of Option<String>: {} bytes", mem::size_of::<Option<String>>());
...
fn demonstrate_complete_ring_buffer() {
    ...
    println!("      Initial state: empty={}, full={}, len={}", 
             queue.is_empty(), queue.is_full(), queue.len());
    ...
    for item in ["A", "B", "C"] {
        ...
        println!("         State: len={}, empty={}, full={}", 
                 queue.len(), queue.is_empty(), queue.is_full());
    }
...
fn demonstrate_real_world_usage() {
    ...
    while packet_buffer.has_packets() {
        if let Some(packet) = packet_buffer.process_next_packet() {
            println!("      📤 Processing packet {} ({} bytes)", 
    ...
    let _ = msg_queue.send(Message::Image { width: 1920, height: 1080, data: vec![0; 1000] });
    let _ = msg_queue.send(Message::Video { duration: 30.0, fps: 60 });
    
    // Receive and process messages
    while let Some(message) = msg_queue.receive() {
        match message {
            Message::Text(text) => println!("      📝 Text message: {}", text),
            Message::Image { width, height, .. } => println!("      🖼️  Image: {}x{}", width, height),
            Message::Video { duration, fps } => println!("      🎥 Video: {:.1}s @ {}fps", duration, fps),
        }
    }
