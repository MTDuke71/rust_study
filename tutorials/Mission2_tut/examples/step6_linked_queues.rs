...
fn demonstrate_simple_linked_queue() {
    ...
    while let Some(item) = queue.dequeue() {
        println!("      Dequeued: '{}', remaining length: {}", item, queue.len());
    }
...
fn demonstrate_optimized_linked_queue() {
    ...
    println!("      Total time for {} dequeues: {:?}", dequeued_count, dequeue_time);
...
fn demonstrate_memory_management() {
    ...
    let node3 = Box::new(Node { data: "Third", next: None });
    let node2 = Box::new(Node { data: "Second", next: Some(node3) });
    let node1 = Box::new(Node { data: "First", next: Some(node2) });
    ...
    println!("      Size of Box<Node<i32>>: {} bytes", mem::size_of::<Box<Node<i32>>>());
    println!("      Size of Node<i32>: {} bytes", mem::size_of::<Node<i32>>());
    println!("      Size of Option<Box<Node<i32>>>: {} bytes", mem::size_of::<Option<Box<Node<i32>>>>());
    
    println!("\n      Memory overhead per node:");
    let data_size = mem::size_of::<i32>();
    let node_size = mem::size_of::<Node<i32>>();
    let overhead = node_size - data_size;
    println!("      Data: {} bytes, Node: {} bytes, Overhead: {} bytes", 
    ...
    impl<T> SimpleQueue<T> {
        fn new() -> Self {
            SimpleQueue { head: None, length: 0 }
        }
    ...
    for i in 1..=3 {
        queue.push(i);
        println!("      After push({}): length = {} (allocation #{} occurred)", 
                 i, queue.len(), i);
    }
    
    // Each pop deallocates a node
    while let Some(value) = queue.pop() {
        println!("      After pop(): value = {}, length = {} (deallocation occurred)", 
                 value, queue.len());
    }
...
fn benchmark_implementations() {
    ...
    impl<T> LinkedQueue<T> {
        ...
        fn enqueue(&mut self, item: T) {
            let new_node = Box::new(Node { data: item, next: None });
            let new_node_ptr = new_node.as_ref() as *const Node<T> as *mut Node<T>;
    ...
    println!("      LinkedQueue overhead: {:.1}x more memory per element", overhead);
