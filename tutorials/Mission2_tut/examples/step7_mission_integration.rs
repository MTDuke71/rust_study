...
fn solve_real_world_problems() {
    ...
    enum GameMessage {
        ...
        EnemySpawn { enemy_type: String, position: (f32, f32) },
        CollectItem { player_id: u32, item_id: u32 },
        PlayerDeath { player_id: u32 },
    }
    ...
    let _ = game_queue.enqueue(GameMessage::PlayerMove { player_id: 1, x: 10.0, y: 20.0 });
    let _ = game_queue.enqueue(GameMessage::EnemySpawn { 
        enemy_type: "Goblin".to_string(), 
        position: (15.0, 25.0) 
    });
    let _ = game_queue.enqueue(GameMessage::CollectItem { player_id: 1, item_id: 42 });
    
    println!("      ✅ Queued 3 game messages");
    
    // Process frame
    let frame_messages = game_queue.process_frame();
    println!("      📦 Processed {} messages this frame", frame_messages.len());
    ...
    for message in frame_messages {
        match message {
            ...
            GameMessage::EnemySpawn { enemy_type, position } => 
    ...
    while let Some(packet) = packet_buffer.process_packet() {
        processed += 1;
        println!("      ⚡ Processed packet from {:?} ({} bytes)", 
                 packet.source_ip, packet.payload.len());
    }
    
    let (bytes, dropped, remaining) = packet_buffer.stats();
    println!("      📊 Stats: {} bytes processed, {} dropped, {} remaining", 
    ...
    impl SensorBuffer {
        ...
        fn get_latest_readings(&self, count: usize) -> Vec<&SensorReading> {
            ...
            let mut current = if self.length == 0 { return readings; } else {
    ...
    for i in 0..5 {
        ...
        match sensor_buffer.add_reading(reading.clone()) {
            Ok(()) => println!("      🌡️  Added reading: sensor {} = {:.1}°C", 
    ...
    println!("      ✅ Domain-specific operations (frame processing, age filtering) build on core queue");
}
...
fn demonstrate_advanced_patterns() {
    ...
    for i in 1..=8 {
        ...
        println!("      📝 Added entry {}, current: {:?}, overwrites: {}", 
                 i, items, log_queue.overwrite_count());
    }
...
fn demonstrate_ecosystem_integration() {
    ...
    println!("      📊 Queue1 length: {}, Queue2 length: {}", 
             queue.items.len(), queue2.items.len());
...
fn discuss_production_deployment() {
    ...
    println!("      📊 Queue Health: {:.1}% utilization, {} errors, {} dropped", 
             health.utilization * 100.0, health.error_count, health.dropped_items);
