...
fn demo_case_insensitive_string() {
    ...
    println!(
        "   Unique tags: {} (case-insensitive deduplication)",
        tags.len()
    );
    for tag in &tags {
        println!("     - {:?}", tag.0);
...
fn demo_custom_point_hash() {
    ...
    }); // Same coords, different label

    println!("   Points: (0,0,'origin'), (1,1,'diagonal'), (0,0,'start')");
    println!(
        "   Unique positions: {} (labels don't matter)",
        visited.len()
    );
...
fn demo_selective_field_hashing() {
    ...
    println!(
        "   Unique users: {} (username-based uniqueness)",
        users.len()
    );
    for user in &users {
...
fn demo_hash_collision_example() {
    ...
    println!(
        "   Bad hash (all collide): {} items stored correctly",
        bad_set.len()
    );
    println!(
        "   Good hash (distributed): {} items stored correctly",
        good_set.len()
    );
