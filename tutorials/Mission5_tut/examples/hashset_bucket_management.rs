...
fn demo_initial_capacity() {
    ...
    println!("   - Capacity: {} (initial allocation)", empty_set.capacity());
...
fn demo_growth_during_insertion() {
    ...
    for i in 0..20 {
        ...
        if current_capacity != previous_capacity {
            println!("   📈 After {} items: capacity {} → {} (GROWTH!)", 
                     i + 1, previous_capacity, current_capacity);
    ...
    println!("   - Load factor: {:.2} ({}/{})", 
             set.len() as f64 / set.capacity() as f64, set.len(), set.capacity());
    println!();
}
...
fn demo_load_factor_concept() {
    ...
    for i in 0..50 {
        ...
        if i == 0 || load_factor > 0.7 || (i + 1) % 10 == 0 {
            println!("   {} items: capacity={}, load_factor={:.3}", 
                     set.len(), set.capacity(), load_factor);
        }
    }
    println!();
}
...
fn demo_capacity_vs_len() {
    ...
    println!("   - len(): {} (items stored)", set.len());
    println!("   - capacity(): {} (bucket slots - doesn't shrink automatically)", set.capacity());
    println!();
}
...
fn demo_shrinking_behavior() {
    ...
    println!("   - Length: {}", set.len());
    println!("   - Capacity: {} (unchanged - no automatic shrinking)", set.capacity());
...
fn demo_rehashing_process() {
    ...
    for (i, item) in items.iter().enumerate() {
        ...
        if new_capacity > old_capacity {
            println!("   🔄 REHASHING at item {}: capacity {} → {}", 
                     i + 1, old_capacity, new_capacity);
            
            // Show hash call counts after potential rehashing
            let total_hash_calls: usize = items.iter()
                .take(i + 1)
                .map(|item| item.hash_call_count())
                .sum();
            
            println!("      Total hash calls so far: {} (includes rehashing)", 
                     total_hash_calls);
        }
    }
...
mod tests {
    ...
    fn test_capacity_behavior() {
        ...
        assert!(set.capacity() >= set.len(), "Capacity should always be >= length");
    }
...
fn demonstrate_bucket_math() {
    ...
    for item in items {
        ...
        println!("   item {} → bucket {} (simplified: {} % {} = {})", 
    ...
    for item in items {
        ...
        println!("   item {} → bucket {} (was bucket {})", 
