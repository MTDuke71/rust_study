...
fn example_basic_multi_value() {
    ...
    let math_students: Vec<_> = student_courses
        .iter()
...
fn example_graph_adjacency() {
    ...
    if let Some((hub_city, connections)) = graph
        .iter()
        .max_by_key(|(_, connections)| connections.len())
    {
        println!(
            "\n🏙️  Major hub: {} ({} connections)",
            hub_city,
            connections.len()
        );
    }
...
fn example_reverse_index() {
    ...
    if let Some(rust_docs) = word_index.get("rust") {
        if let Some(prog_docs) = word_index.get("programming") {
            let intersection: Vec<_> = rust_docs
                .iter()
...
fn example_advanced_operations() {
    ...
    tags_map.add_all(
        "article3",
        vec!["python", "data-science", "machine-learning"],
    );
    ...
    println!(
        "  article1 has 'rust' tag: {}",
        tags_map.contains_value(&"article1", &"rust")
    );
    println!(
        "  article2 has 'rust' tag: {}",
        tags_map.contains_value(&"article2", &"rust")
    );
...
fn exercise_social_network() {
    ...
    if let (Some(alice_friends), Some(bob_friends)) = (friends.get("Alice"), friends.get("Bob")) {
        let mutual: Vec<_> = alice_friends
            .iter()
    ...
    if let Some((popular_person, friend_list)) =
        friends.iter().max_by_key(|(_, friends)| friends.len())
    {
        println!(
            "👑 Most popular: {} with {} friends",
            popular_person,
            friend_list.len()
        );
    }
...
fn exercise_course_prerequisites() {
    ...
    let entry_level: Vec<_> = all_mentioned_courses
        .iter()
    ...
    println!(
        "\n📝 Courses available after completing CS101 and MATH101: {:?}",
        available_courses
    );
    ...
    println!(
        "\n⏱️  Minimum semesters to complete CS401: {}",
        semesters_for_cs401
    );
...
fn exercise_file_system() {
    ...
    fn list_files_recursive(path: &str, fs: &HashMap<&str, Vec<&str>>, files: &mut Vec<String>) {
    ...
    println!(
        "\n🏔️  Deepest directory path: {} (depth: {})",
        deepest_path, depth
    );

    // Bonus: Find all paths to a specific file
    fn find_file_paths(filename: &str, fs: &HashMap<&str, Vec<&str>>) -> Vec<String> {
