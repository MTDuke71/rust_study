...
fn main() {
    ...
    println!("  HashMap: map.get(\"player1\").copied().unwrap_or(\"default\") = {}", score1);
    ...
    multi_map.entry("fruits").or_insert_with(Vec::new).push("apple".to_string());
    multi_map.entry("fruits").or_insert_with(Vec::new).push("banana".to_string());
