...
fn main() {
    ...
    let result = text.chars().enumerate().fold(
        (String::new(), String::new()),
        |(mut even, mut odd), (i, c)| {
            if i % 2 == 0 {
                even.push(c);
            } else {
                odd.push(c);
    ...
    let (even_pairs, odd_pairs): (Vec<_>, Vec<_>) =
        chars.into_iter().partition(|(i, _)| i % 2 == 0);
...
mod tests {
    ...
    fn test_split_even_odd() {
        assert_eq!(
            split_even_odd("abcdef"),
            ("ace".to_string(), "bdf".to_string())
        );
        assert_eq!(
            split_even_odd("12345"),
            ("135".to_string(), "24".to_string())
        );
