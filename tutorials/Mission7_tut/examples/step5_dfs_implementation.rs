...
fn cycle_detection_with_dfs() {
    ...
    println!("  Graph with cycle: {}", if has_cycle1 { "Has cycle" } else { "No cycle" });
    println!("  Graph without cycle: {}", if has_cycle2 { "Has cycle" } else { "No cycle" });
    println!();
...
fn component_analysis_with_dfs() {
    ...
    println!("  Smallest component size: {}", components.iter().map(|c| c.len()).min().unwrap());
    println!();
