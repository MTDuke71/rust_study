...
fn building_adjacency_lists() {
    ...
    println!("  nodes: [Some(\"A\"), Some(\"B\")]");
    println!(
        "  Node A's adjacency list now contains: {:?}",
        graph.neighbors(a)
    );
    println!();
...
fn node_storage_patterns() {
    ...
    let alice = graph3.add_node(Person {
        name: "Alice".to_string(),
        age: 25,
    });
    let bob = graph3.add_node(Person {
        name: "Bob".to_string(),
        age: 30,
    });
...
fn adjacency_list_operations() {
    ...
    println!(
        "  After removing A -> C: A's neighbors: {:?}",
        graph.neighbors(a)
    );
    ...
    graph.add_edge(a, c);
    println!(
        "  After adding A -> C back: A's neighbors: {:?}",
        graph.neighbors(a)
    );
    println!("  Edge count: {}", graph.edge_count());
    println!();
...
fn representation_comparison() {
    ...
    println!(
        "  List is {:.1}x more memory efficient",
        matrix_memory as f64 / list_memory as f64
    );
    println!();
