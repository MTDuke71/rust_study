...
fn social_network_analysis() {
    ...
    let mut degrees: Vec<(NodeId, usize)> =
        network.nodes().map(|id| (id, network.degree(id))).collect();
    ...
    for (i, component) in components.iter().enumerate() {
        let group_names: Vec<String> = component
            .iter()
...
fn performance_optimization() {
    ...
    println!(
        "  Memory per node: {:.1} bytes",
        total_memory as f64 / large_graph.node_count() as f64
    );
    println!();
    ...
    println!(
        "  BFS complexity: O(V + E) = O({})",
        large_graph.node_count() + large_graph.edge_count()
    );
    println!();
...
fn mission7_integration() {
    ...
    println!(
        "  Has edge Start -> Process1: {}",
        graph.has_edge(start, process1)
    );
