...
fn demo_weighted_union_find() {
    ...
    if let Some(dist_03) = wuf.distance(0, 3) {
        if let Some(dist_30) = wuf.distance(3, 0) {
            println!(
                "0→3: {}, 3→0: {} (opposite signs, same magnitude)",
                dist_03, dist_30
            );
        }
    }
...
fn demo_union_find_with_undo() {
    ...
    println!(
        "\nInitial state: {} separate components",
        uuf.component_count()
    );
    ...
    uuf.union(0, 1);
    println!(
        "Connected 0-1. Components: {}, Undo available: {}",
        uuf.component_count(),
        uuf.undo_count()
    );
    ...
    uuf.union(1, 2);
    println!(
        "Connected 1-2. Components: {}, Undo available: {}",
        uuf.component_count(),
        uuf.undo_count()
    );
    ...
    uuf.union(3, 4);
    println!(
        "Connected 3-4. Components: {}, Undo available: {}",
        uuf.component_count(),
        uuf.undo_count()
    );
    ...
    if uuf.undo() {
        println!(
            "Undone! Components: {}, Undo available: {}",
            uuf.component_count(),
            uuf.undo_count()
        );
    }
    ...
    uuf.union(2, 5);
    println!(
        "Connected 2-5 instead. Components: {}",
        uuf.component_count()
    );
    ...
    println!(
        "Back to initial state: {} components",
        uuf.component_count()
    );
}
...
fn demo_persistent_union_find() {
    ...
    let v1 = v0.union(0, 1);
    println!(
        "Version {}: {} components (connected 0-1)",
        v1.version(),
        v1.component_count()
    );
    ...
    let v2 = v1.union(1, 2);
    println!(
        "Version {}: {} components (connected 1-2)",
        v2.version(),
        v2.component_count()
    );
    ...
    println!(
        "Version {}: {} components (alternative: connected 2-3)",
        v1b.version(),
        v1b.component_count()
    );
    ...
    println!(
        "Version {}: {} components (connected 3-4)",
        v2b.version(),
        v2b.component_count()
    );
    ...
    println!("\nComparing versions:");
    println!(
        "v1: 0-1 connected = {}, 2-3 connected = {}",
        v1.connected(0, 1),
        v1.connected(2, 3)
    );
    println!(
        "v1b: 0-1 connected = {}, 2-3 connected = {}",
        v1b.connected(0, 1),
        v1b.connected(2, 3)
    );
...
fn demo_customizable_merge() {
    ...
    println!(
        "After union(0,1): size = {}, sum = {}, max = {}, min = {}",
    ...
    println!(
        "After union(2,3): size = {}, sum = {}, max = {}, min = {}",
        stats.size, stats.sum, stats.max, stats.min
    );
    ...
    let stats = cuf.get_set_data(0);
    println!(
        "After union(0,2): size = {}, sum = {}, max = {}, min = {}",
        stats.size, stats.sum, stats.max, stats.min
    );
    ...
    for i in 0..4 {
        let stats = cuf.get_set_data(i);
        println!(
            "Via element {}: sum = {} (avg = {:.1})",
            i,
            stats.sum,
            stats.sum as f64 / stats.size as f64
        );
    }
...
mod tests {
    ...
    fn test_customizable_merge() {
        let initial_stats = vec![SetStats::new(10), SetStats::new(20), SetStats::new(30)];
        let mut cuf = CustomizableUnionFind::new(initial_stats, SetStats::merge);
