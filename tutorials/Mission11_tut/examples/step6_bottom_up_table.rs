...
fn pattern_1_string_problems() {
    ...
    println!(
        "  Top-down: {} (natural - matches problem structure)",
        result_td
    );
    ...
    println!(
        "  Bottom-up: {} (complex - need substring ordering)",
        result_bu
    );
...
fn pattern_2_grid_paths() {
    ...
    println!(
        "  Problem: Count paths from (0,0) to ({},{}) (only right/down)",
        rows - 1,
        cols - 1
    );
    ...
    println!(
        "  Bottom-up: {} paths (natural - fill grid left-to-right, top-to-bottom)",
        result_bu
    );
...
fn count_paths_top_down(row: usize, col: usize, memo: &mut HashMap<(usize, usize), u64>) -> u64 {
    ...
    let result =
        count_paths_top_down(row - 1, col, memo) + count_paths_top_down(row, col - 1, memo);
...
fn stack_overflow_risk() {
    ...
    println!(
        "    Stack depth: ~{} calls (each subproblem computed once)",
        medium_n
    );
