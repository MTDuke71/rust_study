...
fn main() {
    ...
    println!(
        "💡 Key Insight: Without caching, we solve the same subproblem exponentially many times!"
    );
}
...
fn demonstrate_core_concept() {
    ...
    println!(
        "Problem: Can we make '{}' from patterns {:?}?",
        design, patterns
    );
    ...
    println!(
        "Key Insight: Each recursive call checks if a prefix matches, then recurses on remainder"
    );
}
...
fn pattern_3_multiple_solutions() {
    ...
    let result = can_make(&patterns, design);
    println!(
        "  '{}' can be made: {} (has multiple paths)",
        design, result
    );
