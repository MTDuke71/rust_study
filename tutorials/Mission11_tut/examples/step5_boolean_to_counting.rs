...
fn demonstrate_core_concept() {
    ...
    println!(
        "   Cache size: {} (stores true/false for each substring)",
        memo_bool.len()
    );
    ...
    println!(
        "   Cache size: {} (stores count for each substring)",
        memo_count.len()
    );
...
fn empty_string_case() {
    ...
    println!(
        "  Counting: {} way (exactly one way to make nothing)",
        count
    );
