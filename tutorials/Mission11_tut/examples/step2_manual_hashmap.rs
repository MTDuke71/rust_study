...
fn pattern_1_cache_statistics() {
    ...
    println!("  Max possible substrings: {}", design.len() * (design.len() + 1) / 2);
...
fn pattern_2_progressive_difficulty() {
    ...
    for design in test_cases {
        ...
        println!("  Length {}: {:?}, cache size: {}, result: {}", 
                 design.len(), duration, memo.len(), result);
    }
}
...
fn pattern_3_cache_reuse() {
    ...
    for design in designs {
        ...
        println!("    '{}': {} (cache grew by {} entries)", 
                 design, result, cache_size_after - cache_size_before);
    }
    
    println!("  Final cache size: {} (contains results for all substrings seen)", memo.len());
}
...
fn empty_design_cached() {
    ...
    println!("  Result: {}", result);
    println!("  Cache size: {} (empty string returns immediately)", memo.len());
...
fn validate_understanding() {
    ...
    println!("  Cache entries: {} (note: 'rr' appears multiple times in recursion)", memo2.len());
