...
fn pattern_2_string_problems() {
    ...
    println!(
        "  String length caching: {} (demonstrating flexibility)",
        count
    );
}
...
fn pattern_3_cache_statistics_analysis() {
    ...
    let test_cases = vec![("r", 1), ("wrr", 3), ("brwrr", 5), ("brwrrbrwrr", 10)];
...
fn demonstrate_custom_problem() {
    ...
    for n in [5, 10, 5, 12, 10] {
        ...
        println!(
            "  factorial({:2}) = {:10} | hits: {:2}, misses: {:2}",
