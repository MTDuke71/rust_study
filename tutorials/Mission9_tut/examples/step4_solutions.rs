    ...
    pub fn find_path(
        ...
        while let Some(current) = open_set.pop() {
            ...
            let current_weight = self
                .weighting_strategy
                .get_weight(self.stats.nodes_explored, estimated_max_nodes);
...
impl SequentialBidirectionalAStar {
    ...
    fn search_forward(
        ...
        start: Position,
        goal: Position,
    ) -> (usize, usize, Option<Vec<Position>>) {
...
impl HierarchicalPathfinding {
    ...
    pub fn find_path(
        ...
        let entrance_1 = Position::new(
            cluster_start_x + self.cluster_size / 2,
            cluster_start_y + self.cluster_size / 2,
        );
        let entrance_2 = Position::new(
            cluster_goal_x + self.cluster_size / 2,
            cluster_goal_y + self.cluster_size / 2,
        );
    ...
    pub fn select_algorithm(characteristics: &ProblemCharacteristics) -> SelectedAlgorithm {
        ...
        match characteristics {
            ...
            _c if grid_area > 50000 => {
                ...
                println!(
                    "🎯 Selected: Hierarchical Pathfinding (very large grid, cluster_size={})",
                    cluster_size
                );
...
pub fn main() {
    ...
    println!(
        "   Grid size: {}x{}",
        characteristics.grid_size.0, characteristics.grid_size.1
    );
    println!(
        "   Obstacle density: {:.1}%",
        characteristics.obstacle_density * 100.0
    );
    println!(
        "   Goal distance: {} Manhattan units",
        characteristics.goal_distance
    );
    println!(
        "   Memory budget: {} KB",
        characteristics.available_memory / 1024
    );
