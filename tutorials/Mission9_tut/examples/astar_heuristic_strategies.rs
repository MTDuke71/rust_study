...
fn calculate_f_score(
    ...
    match strategy {
        HeuristicStrategy::Standard => g + h,

        HeuristicStrategy::Weighted(weight) => g + (*weight * h as f64) as usize,

        HeuristicStrategy::Dynamic {
            initial_weight,
            max_depth,
        } => {
        ...
        HeuristicStrategy::Focal {
            suboptimality_bound: _,
        } => {
...
fn demonstrate_admissibility() {
    ...
    for (pos, name) in positions {
        ...
        let euclidean = (((pos.0 as f64 - goal.0 as f64).powi(2)
            + (pos.1 as f64 - goal.1 as f64).powi(2))
        .sqrt()) as usize;
