...
impl ContractionHierarchy {
    ...
    fn build(graph: PreprocessedGraph, strategy: ImportanceStrategy) -> Self {
        ...
        println!("   Shortcuts created: {}", shortcuts.values().map(|v| v.len()).sum::<usize>());
    ...
    fn contract_node(
        ...
        let neighbors: Vec<(usize, f64)> = graph.neighbors(node)
            .iter()
    ...
    fn query(&self, start: usize, goal: usize) -> Option<(f64, Vec<usize>)> {
        ...
        forward_queue.push(State { cost: 0.0, node: start });
        
        backward_dist[goal] = 0.0;
        backward_queue.push(State { cost: 0.0, node: goal });
        ...
        while !forward_queue.is_empty() || !backward_queue.is_empty() {
            ...
            if let Some(State { cost, node }) = forward_queue.pop() {
                ...
                self.explore_upward_edges(node, cost, &mut forward_dist, &mut forward_parent, &mut forward_queue);
            }
            ...
            if let Some(State { cost, node }) = backward_queue.pop() {
                ...
                self.explore_upward_edges_reverse(node, cost, &mut backward_dist, &mut backward_parent, &mut backward_queue);
            }
        }
            ...
            if self.node_rank[from] > self.node_rank[node] {
                ...
                if let Some(weight) = self.base_graph.edge_weight(from, node) {
                    ...
                    if new_cost < dist[from] {
                        ...
                        queue.push(State { cost: new_cost, node: from });
                    }
                }
...
fn demo_graph_preprocessing() {
    ...
    println!("  Nodes: {}", graph.node_count);
    println!("  Edges: {}", graph.edges.values().map(|v| v.len()).sum::<usize>());
