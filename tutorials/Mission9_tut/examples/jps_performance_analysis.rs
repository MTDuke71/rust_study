...
impl InstrumentedJPS {
    ...
    fn get_stats(&self) -> (u32, u32, u32) {
        (
            self.jump_calls,
            self.walkable_checks,
            self.forced_neighbor_checks,
        )
    }
}
...
pub fn main() {
    ...
    println!(
        "\n💡 JPS Computational Overhead: {:.1}× more operations per expansion",
        overhead_multiplier
    );
    ...
    println!(
        "   • Standard A*: 81 × {} = {} total operations",
        total_astar_operations,
        81 * total_astar_operations
    );
    println!(
        "   • JPS (same ops/node): 12 × {} = {} total operations",
        total_jps_operations,
        12 * total_jps_operations
    );
    ...
    if jps_projected_ops > astar_projected_ops {
        ...
        println!(
            "   • JPS does {:.1}× MORE total work despite exploring fewer nodes!",
            ops_ratio
        );
    }
