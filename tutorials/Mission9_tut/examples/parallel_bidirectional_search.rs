            ...
            if closed.contains(&node_id) {
                if let Some(&other_cost) = g_scores.get(&node_id) {
                    ...
                    if total_cost < current_best {
                        self.best_meeting_cost
                            .store(total_cost, AtomicOrdering::Release);
                        *self.meeting_point.lock().unwrap() = Some(node_id);
                        println!(
                            "   🎯 Meeting point found at node {} with cost {}",
                            node_id, total_cost
                        );
                        return true;
                    }
    ...
    fn search_direction(
        ...
        while let Some(current) = open_set.pop() {
            ...
            if shared_state.should_terminate(current.f_score()) {
                println!(
                    "   🛑 {} thread terminating early (f-score: {})",
                    thread_name,
                    current.f_score()
                );
                break;
            ...
            if nodes_explored % 50 == 0 {
                println!(
                    "   📈 {} thread: {} nodes explored",
                    thread_name, nodes_explored
                );
            }
        }

        println!(
            "   🏁 {} thread finished: {} explored, {} generated",
