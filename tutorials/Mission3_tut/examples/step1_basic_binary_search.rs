...
fn main() {
    ...
    match basic_binary_search(&sorted_data, &target) {
        ...
        Err(insert_pos) => {
            ...
            println!("   This maintains sorted order: before {:?}, after would be {:?}",
    ...
    let single = [42];
    println!("Single element (found): {:?}", basic_binary_search(&single, &42));
    
    // Single element - not found
    println!("Single element (not found): {:?}", basic_binary_search(&single, &10));
    ...
    println!("String search: {:?}", basic_binary_search(&words, &"cherry"));
    
    // Works with tuples (sorted by first element)
    let pairs = [(1, "one"), (2, "two"), (3, "three"), (5, "five")];
    println!("Tuple search: {:?}", basic_binary_search(&pairs, &(3, "three")));
    ...
    println!("Binary search found {} at {:?} with {} comparisons", 
    ...
    println!("Linear search found {} at {:?} with {} comparisons",
    ...
    println!("   Binary search result: {:?} (WRONG!)", 
             basic_binary_search(&unsorted, &7));
    
    let mut sorted = unsorted.clone();
    sorted.sort();
    println!("   After sorting: {:?}", sorted);
    println!("   Binary search result: {:?} (Correct!)", 
...
fn binary_search_with_trace<T: Ord + std::fmt::Debug>(slice: &[T], target: &T) {
    ...
    println!("Initial: left={}, right={} (search space: {:?})", 
    ...
    while left < right {
        ...
        println!("   New range: left={}, right={} (search space: {:?})", 
                 left, right, &slice[left..right.min(slice.len())]);
        step += 1;
    }
...
fn binary_search_with_count<T: Ord>(slice: &[T], target: &T, count: &mut usize) 
    -> Result<usize, usize> 
{
    let mut left = 0;
    let mut right = slice.len();
...
/// Linear search for comparison
fn linear_search_with_count<T: Ord>(slice: &[T], target: &T, count: &mut usize) 
    -> Option<usize> 
{
