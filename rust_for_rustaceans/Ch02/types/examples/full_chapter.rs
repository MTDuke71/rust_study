...
impl DataPipeline<Validated> {
    ...
    fn process_iterator(&self) -> impl Iterator<Item = i32> + '_ {
        self.data.iter().map(|&x| x * 2).filter(|&x| x > 5)
    }
}
...
impl DataPipeline<Processed> {
    fn display_results(&self) {
        ...
        let formatters: Vec<Box<dyn ResultFormatter>> =
            vec![Box::new(SimpleFormatter), Box::new(DetailedFormatter)];
...
fn test_memory_layout() {
    ...
    println!(
        ...
        data.id, data.count, data.active
    );
