...
async fn main() {
    ...
    let results: Vec<_> = stream::iter(1..=5).then(process_item).collect().await;
    ...
    let batches = stream::iter(1..=10).ready_chunks(3); // Collect chunks of 3
    ...
    let throttled = stream::iter(1..=5).then(|n| async move {
    ...
    println!(
        "Time: {:?} (enforced 200ms between items)\n",
        start.elapsed()
    );
