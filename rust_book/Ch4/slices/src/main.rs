...
// The first_word function has a parameter of type &String. We don’t need ownership, so this is fine. (In idiomatic Rust, functions do not take ownership of their arguments unless they need to, and the reasons for that will become clear as we keep going.) 
//But what should we return? We don’t really have a way to talk about part of a string. However, we could return the index of the end of the word, indicated by a space. Let’s try that, as shown in Listing 4-7.
...
//     for (i, &item) in bytes.iter().enumerate() {
// We’ll discuss iterators in more detail in Chapter 13. For now, know that iter is a method that returns each element in a collection and that enumerate wraps the result of iter and returns each element as part of a tuple instead. 
...
// Because the enumerate method returns a tuple, we can use patterns to destructure that tuple. We’ll be discussing patterns more in Chapter 6. In the for loop, we specify a pattern that has i for the index in the tuple and &item for the single byte in the tuple. 
...
//In other words, because it’s a separate value from the String, there’s no guarantee that it will still be valid in the future. Consider the program in Listing 4-8 that uses the first_word function from Listing 4-7.
...
//The problems would show up later if we kept trying to use the first word index with an emptied string. Slices make this bug impossible and let us know we have a problem with our code much sooner. 
...
fn main() {
    ...
    // Adding fn second_word(s: &String) -> (usize, usize) {} would just increase the problem 
    ...
    let slice2 = &s[0..2];
    let slice3 = &s[..2];  //same as slice2
    println!("the first two letters are: {slice2}, {slice3}");

    let len = s.len();

    let slice4 = &s[3..len];
    let slice5 = &s[3..];  //same as slice4
    println!("the last letters are: {slice4}, {slice5}");

    let s = String::from("hello");
    let len = s.len();

    let slice6 = &s[0..len];
    let slice7 = &s[..]; //same as slice6
    println!("the whole word is: {slice6}, {slice7}");

    // Fixes invalid word problem
    // let mut s = String::from("hello world");
