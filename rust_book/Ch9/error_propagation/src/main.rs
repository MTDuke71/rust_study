...
fn demonstrate_option_propagation() {
    ...
    for (i, numbers) in test_cases.iter().enumerate() {
        match calculate_average(numbers) {
            Some(avg) => println!("   Case {}: {:?} -> Average of first/last: {:.1}", 
                               i + 1, numbers, avg),
            None => println!("   Case {}: {:?} -> None (empty or invalid)", 
                           i + 1, numbers),
        }
    }
...
fn demonstrate_custom_error_conversions() {
    ...
    fn process_user_file(filename: &str) -> Result<u32, ProcessingError> {
        ...
        let number: u32 = content.trim().parse()
            .map_err(|_| ProcessingError::Parse(format!("Invalid number in {}", filename)))?;
        
        // Business logic validation
        if number == 0 {
            return Err(ProcessingError::Business("Number cannot be zero".to_string()));
        }
...
fn demonstrate_operation_chaining() {
    ...
    fn process_pipeline(input: &str) -> Result<String, ProcessingError> {
        ...
        Ok(format!("Numbers: {:?}, Sum: {}, Average: {:.2}", numbers, sum, average))
    }
...
fn cleanup_test_files() {
    let files_to_remove = vec![
        "username.txt", "config.txt", "output.txt", 
        "valid_number.txt", "zero_number.txt", "large_number.txt", 
        "invalid_number.txt", "numbers.txt"
    ];
...
mod tests {
    ...
    fn test_custom_error_conversion() {
        ...
        assert!(matches!(parse_number("abc"), Err(ProcessingError::Parse(_))));
    }
}