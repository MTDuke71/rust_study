...
fn demonstrate_result_combinators() {
    ...
    fn combine_results() -> Result<String, String> {
        let num1 = "42".parse::<i32>().map_err(|_| "First parse failed".to_string())?;
        let num2 = "24".parse::<i32>().map_err(|_| "Second parse failed".to_string())?;
...
mod tests {
    ...
    fn test_error_transformation() {
        ...
        assert_eq!(parse_with_context("abc"), Err("Failed to parse 'abc'".to_string()));
    }
}