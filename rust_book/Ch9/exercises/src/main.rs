...
fn exercise_2_file_operations() {
    ...
    match read_file_safe("test_file.txt") {
        Ok(content) => println!("  ✅ File content ({} chars): {}", content.len(), 
...
fn exercise_4_error_propagation() {
    ...
    fn validate_and_process_config() -> Result<String, String> {
        ...
        for (line_num, line) in config_data.lines().enumerate() {
            ...
            if parts.len() != 2 {
                return Err(format!("Invalid config format on line {}: '{}'", line_num + 1, line));
            }
    ...
    let valid_config = "# Application Configuration\nhost=localhost\nport=8080\ndatabase=myapp\ndebug=true";
    ...
    impl fmt::Display for ConfigError {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            match self {
                ...
                ConfigError::ParseError { line, message } => write!(f, "Parse error on line {}: {}", line, message),
    ...
    impl AppConfig {
        fn from_file(filename: &str) -> Result<Self, ConfigError> {
            ...
            let max_connections: u32 = max_conn_str.parse()
                .map_err(|_| ConfigError::ValidationError(format!("Invalid max_connections: {}", max_conn_str)))?;
            
            let timeout_str = get_required("timeout_seconds")?;
            let timeout_seconds: u64 = timeout_str.parse()
                .map_err(|_| ConfigError::ValidationError(format!("Invalid timeout_seconds: {}", timeout_str)))?;
            
            let debug_str = config_map.get("debug_mode").unwrap_or(&"false".to_string()).clone();
            let debug_mode: bool = debug_str.parse()
                .map_err(|_| ConfigError::ValidationError(format!("Invalid debug_mode: {}", debug_str)))?;
            
            // Business logic validation
            if port < 1024 {
                return Err(ConfigError::ValidationError("Port must be >= 1024".to_string()));
            }
            
            if max_connections == 0 || max_connections > 10000 {
                return Err(ConfigError::ValidationError("max_connections must be 1-10000".to_string()));
            }
            
            if timeout_seconds == 0 || timeout_seconds > 3600 {
                return Err(ConfigError::ValidationError("timeout_seconds must be 1-3600".to_string()));
            }
...
debug_mode=true"#;
    ...
    match AppConfig::from_file("valid_app_config.txt") {
        Ok(config) => println!("  ✅ Valid config loaded: {}@{}:{}", 
...
fn exercise_6_advanced_patterns() {
    ...
    fn load_config_with_fallback(primary: &str, fallback: &str) -> Result<String, String> {
        std::fs::read_to_string(primary)
            .or_else(|_| {
                println!("    Primary config '{}' failed, trying fallback...", primary);
    ...
    let test_files = vec!["test_file.txt", "nonexistent1.txt", "fallback_config.txt", "nonexistent2.txt"];
...
fn cleanup_exercise_files() {
    let files_to_remove = vec![
        "test_file.txt", "test_file_backup.txt", "calc_input.txt", 
        "numbers.txt", "invalid_numbers.txt", "app_config.txt",
        "valid_app_config.txt", "invalid_app_config.txt", "fallback_config.txt"
    ];
...
mod tests {
    ...
    fn test_calculator_errors() {
        ...
        assert!(matches!(Calculator::power(2.0, 1000.0), Err(CalculatorError::Overflow)));
    }
