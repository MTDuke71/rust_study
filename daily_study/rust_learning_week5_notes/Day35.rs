...
impl CsvParser {
        ...
        T: FromCsvRow,
    {
        ...
        // line_number is used in error handling contexts
            ...
            match self.parse_row(line, &headers, line_number) {
                ...
                Err(mut error) => {
                    if self.error_recovery.can_recover(&error) {
                        match self.error_recovery.recover(
                            error.clone(),
                            &ParseContext {
                                line_number,
                                column: 0,
...
impl FromCsvRow for UserRecord {
    fn from_csv_row(data: HashMap<String, String>, line_number: usize) -> Result<Self, ParseError> {
        ...
        let age = data.get("age").and_then(|age_str| {
            if age_str.is_empty() {
                None
            } else {
...
impl ConfigParser {
    ...
    fn parse_config_line(
        ...
        line: &str,
        line_number: usize,
    ) -> Result<(String, ConfigValue), ParseError> {
