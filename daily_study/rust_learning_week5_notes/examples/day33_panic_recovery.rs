...
fn recover_from_panic() {
    ...
    let result = panic::catch_unwind(|| {
        ...
        let x = [1, 2, 3];
        println!("{}", x[10]); // This will panic (index out of bounds) - intentional for demo
    });
...
{
    panic::catch_unwind(operation).map_err(|_| FfiError::Panic)
}
...
impl WebServer {
    ...
    fn handle_request(&self, request: &Request) -> Response {
        let result = panic::catch_unwind(|| self.process_request(request));
        ...
        match result {
            ...
            Err(_) => {
                ...
                self.panic_handler
                    .log_panic(&format!("Request panicked: {:?}", request));
...
impl BatchProcessor {
    ...
    fn process_batch(&self, items: Vec<String>) -> BatchSummary {
        ...
        for (index, item) in items.into_iter().enumerate() {
            let result = panic::catch_unwind(|| self.process_item(item.clone()));
            ...
            match result {
                ...
                Err(_) => {
                    ...
                    self.results
                        .lock()
                        .unwrap()
                        .push(BatchResult::Panic { index, item });
                }
            }
        }
...
impl PluginManager {
    ...
    fn execute_plugin(&self, name: &str, input: &str) -> PluginResult {
        if let Some(plugin) = self.plugins.get(name) {
            let result = panic::catch_unwind(panic::AssertUnwindSafe(|| plugin.execute(input)));
...
fn main() {
    ...
    plugin_manager.register_plugin(
        ...
        Box::new(UnsafePlugin {
            name: "UnsafePlugin".to_string(),
        }),
    );
    ...
    match recovery.execute_with_recovery(|| "Hello, World!".to_string()) {
    ...
    match recovery.execute_with_recovery(|| "Should not execute".to_string()) {
        Ok(value) => println!("Recovery execution succeeded: {}", value),
        Err(e) => println!("Recovery execution failed: {:?}", e),
    }
    println!();
