...
impl std::fmt::Display for FileProcessingError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FileProcessingError::ProcessingFailed { stage, reason } => write!(f, "Processing failed at stage {}: {}", stage, reason),
            FileProcessingError::OutputNotWritable { path } => write!(f, "Output directory not writable: {}", path),
