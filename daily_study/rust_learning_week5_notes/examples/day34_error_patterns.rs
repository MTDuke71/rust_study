...
impl std::fmt::Display for FileProcessingError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            FileProcessingError::PermissionDenied { path } => {
                write!(f, "Permission denied: {}", path)
            }
            FileProcessingError::InvalidFormat { expected, actual } => {
                write!(f, "Invalid format: expected {}, got {}", expected, actual)
            }
            FileProcessingError::ProcessingFailed { stage, reason } => {
                write!(f, "Processing failed at stage {}: {}", stage, reason)
            }
            FileProcessingError::OutputNotWritable { path } => {
                write!(f, "Output directory not writable: {}", path)
            }
...
fn parse_age(input: &str) -> Result<u32, ParseError> {
    ...
    input.parse::<u32>().map_err(|_| ParseError::InvalidFormat)
}
...
impl BankAccount {
    ...
    fn withdraw(&mut self, amount: u64) {
        ...
        if amount > self.balance as u64 {
            panic!(
                "Insufficient funds: attempted to withdraw {} but only have {}",
                amount, self.balance
            );
        }
...
fn process_user_data(user_id: u32, data: &str) -> Result<ProcessedData, String> {
    ...
    let enriched =
        enrich_user_data(validated).map_err(|e| format!("Failed to enrich user data: {:?}", e))?;
...
impl FileProcessor {
    ...
    fn process_single_file(&self, file_path: &str) -> Result<ProcessedFile, FileProcessingError> {
        // Read file
        let content =
            std::fs::read_to_string(file_path).map_err(|_| FileProcessingError::FileNotFound {
    ...
    fn parse_content(
        ...
        content: &str,
        file_path: &str,
    ) -> Result<ParsedData, FileProcessingError> {
        let extension = Path::new(file_path)
            .extension()
            .and_then(|ext| ext.to_str())
            .unwrap_or("");
    ...
    fn write_output(&self, content: &str, output_path: &str) -> Result<(), FileProcessingError> {
        std::fs::write(output_path, content).map_err(|_| FileProcessingError::Io {
