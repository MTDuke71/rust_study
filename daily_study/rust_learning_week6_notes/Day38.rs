...
impl FeaturefulUnionFind {
    ...
    pub fn parallel_batch_union(&mut self, pairs: Vec<(usize, usize)>) -> Vec<bool> {
        ...
        pairs.into_iter().map(|(x, y)| self.union(x, y)).collect()
    }
...
fn main() {
    ...
    #[cfg(not(any(feature = "std", feature = "alloc")))]
    {
        ...
        let result = cache.insert(1, "Hello".to_string());
        println!(
            "insert() returned: {:?} (always None without storage)",
            result
        );
    }
...
mod tests {
    ...
    fn test_serialization() {
        ...
        // Test round-trip
        let _restored =
            FeaturefulUnionFind::from_json(&json).expect("Should deserialize successfully");
    }
