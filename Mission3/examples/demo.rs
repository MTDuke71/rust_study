...
fn basic_slice_search() {
    ...
    for target in [0, 4, 20] {
        match binary_search::search_slice(&numbers, &target) {
            Ok(index) => println!("  ✅ Found {} at index {}", target, index),
            Err(insert_idx) => println!("  📍 {} would be inserted at index {}", target, insert_idx),
        }
    }
...
fn custom_ordering_search() {
    ...
    let items = [(100, "expensive"), (50, "medium"), (25, "cheap"), (75, "moderate")];
    ...
    let mut students = vec![
        Student { name: "Alice".to_string(), grade: 95.5 },
        Student { name: "Bob".to_string(), grade: 87.3 },
        Student { name: "Charlie".to_string(), grade: 92.1 },
        Student { name: "Diana".to_string(), grade: 88.7 },
    ];
    ...
    match binary_search::search_by_key(&students, &target_grade, |s| (s.grade * 10.0) as u32) {
        Ok(idx) => println!("  ✅ Found student with grade ~88.7: {}", students[idx].name),
...
fn lifetime_safety_demo() {
    ...
    let range_refs: Vec<_> = {
        search_iter::find_range(&data, &20, &40).collect()
    };
