...
fn demonstrate_reference_counting() {
    ...
    println!("Initial reference count for first node: {}", Rc::strong_count(&first_node));
    println!("Initial reference count for second node: {}", Rc::strong_count(&second_node));
    ...
    println!("Data via additional ref1: {}", _additional_ref1.borrow().data);
    println!("Data via additional ref2: {}", _additional_ref2.borrow().data);
...
fn demonstrate_shared_access() {
    ...
    println!("After modifying via ref1, ref2 sees: {:?}", node_ref2.borrow().data);
}
