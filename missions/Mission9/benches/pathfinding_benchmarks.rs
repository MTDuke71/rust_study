...
use mission9::{
    SimpleWeightedGraph, DijkstraPathfinder, AstarPathfinder, 
    Pathfinder, EuclideanHeuristic, HeuristicContext
};
...
fn benchmark_dijkstra_small_grid(c: &mut Criterion) {
    ...
    c.bench_function("dijkstra_10x10_grid", |b| {
        b.iter(|| {
            pathfinder.find_path(black_box(&graph), black_box(0), black_box(99))
        })
    });
}
