...
fn test_req3_bidirectional_dijkstra_simple_path() {
    ...
    let path = dijkstra.find_path(start, goal, |pos| grid_neighbors_4(pos, 10, 10));
...
fn test_req3_bidirectional_vs_traditional_efficiency() {
    ...
    let bi_path = bi_astar.find_path(start, goal, manhattan_distance, |pos| {
        grid_neighbors_4(pos, 20, 20)
    });
    ...
    println!(
        "Bidirectional A* explored: {} nodes",
        bi_metrics.nodes_explored
    );

    // Bidirectional should generally explore fewer nodes for distant goals
    assert!(
        bi_metrics.nodes_explored < 300,
        "Bidirectional A* should be efficient"
    );
...
fn test_req4_early_termination_close_goal() {
    ...
    assert!(
        metrics.nodes_explored < 10,
        "Should terminate early for close goals"
    );
    assert!(
        metrics.early_terminations > 0,
        "Should record early termination"
    );
}
...
fn test_memory_optimized_astar_with_different_heuristics() {
    ...
    let path_manhattan = astar_manhattan.find_path(start, goal, manhattan_distance, |pos| {
        grid_neighbors_4(pos, 10, 10)
    });

    // Test with Euclidean distance
    let mut astar_euclidean = MemoryOptimizedAstar::new(1000);
    let path_euclidean = astar_euclidean.find_path(start, goal, euclidean_distance, |pos| {
        grid_neighbors_4(pos, 10, 10)
    });
...
fn test_algorithm_comparison_performance() {
    ...
    let path_bi_manhattan = bi_astar_manhattan.find_path(start, goal, manhattan_distance, |pos| {
        grid_neighbors_4(pos, grid_size, grid_size)
    });
    ...
    let start_time = Instant::now();
    let path_bi_euclidean = bi_astar_euclidean.find_path(start, goal, euclidean_distance, |pos| {
        grid_neighbors_4(pos, grid_size, grid_size)
    });
    ...
    let start_time = Instant::now();
    let path_bi_dijkstra = bi_dijkstra.find_path(start, goal, |pos| {
        grid_neighbors_4(pos, grid_size, grid_size)
    });
    ...
    let start_time = Instant::now();
    let path_mem_astar = mem_astar.find_path(start, goal, manhattan_distance, |pos| {
        grid_neighbors_4(pos, grid_size, grid_size)
    });
    ...
    assert_eq!(
        bi_manhattan_metrics.path_cost,
        bi_euclidean_metrics.path_cost
    );
    assert_eq!(
        bi_manhattan_metrics.path_cost,
        bi_dijkstra_metrics.path_cost
    );
...
fn test_req3_req4_comprehensive_validation() {
    ...
    let bi_path = bi_astar.find_path(start, goal, manhattan_distance, |pos| {
        grid_neighbors_4(pos, 15, 15)
    });

    let mut bi_dijkstra = BidirectionalDijkstra::new(1000);
    let dijkstra_path = bi_dijkstra.find_path(start, goal, |pos| grid_neighbors_4(pos, 15, 15));
    ...
    println!(
        "Bidirectional Dijkstra explored: {} nodes",
    ...
    assert!(
        close_metrics.early_terminations > 0,
        "Should have early termination for close goals"
    );
    assert!(
        close_metrics.nodes_explored < 10,
        "Should explore very few nodes for close goals"
    );
