...
impl<H: crate::Heuristic> ConstrainedAstar<H> {
    ...
    pub fn find_constrained_path(
        ...
        let h_score = self.heuristic.estimate_distance(start, goal, heuristic_context);
        ...
        while let Some(current) = open_set.pop() {
            ...
            for (neighbor, edge_weight) in graph.neighbors(current.node) {
                ...
                let h_score = self.heuristic.estimate_distance(neighbor, goal, heuristic_context);
...
pub mod presets {
    ...
    pub fn avoid_zones<H: crate::Heuristic>(
        ...
        ConstrainedAstar::new(heuristic)
            .add_constraint(PathConstraintType::ForbiddenZone(ForbiddenZoneConstraint::new().add_forbidden_nodes(forbidden_nodes)))
    }
    ...
    pub fn limited_search<H: crate::Heuristic>(
        ...
        ConstrainedAstar::new(heuristic)
            .add_constraint(PathConstraintType::MaxCost(MaxCostConstraint::new(max_cost)))
            .add_constraint(PathConstraintType::MaxLength(MaxLengthConstraint::new(max_length)))
    }
    
    /// Create a pathfinder that prevents cycles
    pub fn acyclic_search<H: crate::Heuristic>(heuristic: H) -> ConstrainedAstar<H> {
        ConstrainedAstar::new(heuristic).add_constraint(PathConstraintType::NoCycle(NoCycleConstraint))
    }
...
mod tests {
    use super::*;
    use crate::{SimpleWeightedGraph, ManhattanHeuristic, HeuristicContext};
    ...
    fn test_max_length_constraint() {
        ...
        let context2 = context1.with_step(1, 1.0).with_step(2, 1.0).with_step(3, 1.0);
    ...
    fn test_constrained_astar_forbidden_zones() {
        ...
        let pathfinder = ConstrainedAstar::new(ManhattanHeuristic)
            .add_constraint(PathConstraintType::ForbiddenZone(ForbiddenZoneConstraint::new().add_forbidden_node(4)));
        
        let result = pathfinder.find_constrained_path(&graph, 0, 8, &heuristic_context).unwrap();
    ...
    fn test_constrained_astar_no_cycles() {
        ...
        for &node in &result.path {
            assert!(!seen_nodes.contains(&node), "Cycle detected in path: {:?}", result.path);
    ...
    fn test_multiple_constraints() {
        ...
        assert!(result_basic.is_ok(), "Basic constrained pathfinding should work");
        
        // Test with just forbidden zone constraint
        let pathfinder_forbidden = ConstrainedAstar::new(ManhattanHeuristic)
            .add_constraint(PathConstraintType::ForbiddenZone(ForbiddenZoneConstraint::new().add_forbidden_node(4)));
        let result_forbidden = pathfinder_forbidden.find_constrained_path(&graph, 0, 8, &heuristic_context);
        ...
        if result.is_err() {
            ...
            println!("No path found with multiple constraints - constraints may be too restrictive");
    ...
    fn test_no_valid_path() {
        ...
        let pathfinder = ConstrainedAstar::new(ManhattanHeuristic)
              .add_constraint(PathConstraintType::ForbiddenZone(ForbiddenZoneConstraint::new()
                .add_forbidden_nodes(&[1, 3, 4, 5, 7])));        let result = pathfinder.find_constrained_path(&graph, 0, 8, &heuristic_context);
