...
fn req4_maze_blocked_path_detection() {
    ...
    assert!(
        path_result.is_err(),
        "Should not find path between isolated islands"
    );
}
...
fn req4_network_connectivity_analysis() {
    ...
    assert_eq!(
        sizes,
        vec![1, 2, 3],
        "Components should be sizes 1, 2, and 3"
    );
}
...
fn req4_dependency_transitive_analysis() {
    ...
    assert!(
        visited_from_app.contains(&2),
        "Should visit HTTP dependency"
    );
    assert!(
        visited_from_app.contains(&3),
        "Should visit Auth dependency"
    );
    assert!(
        visited_from_app.contains(&4),
        "Should visit transitive Utils dependency"
    );
    assert_eq!(
        visited_from_app.len(),
        4,
        "Should visit all 4 packages transitively"
    );
}
...
fn req4_edge_cases_handled_correctly() {
    ...
    assert_eq!(
        single_components.len(),
        1,
        "Single node should form one component"
    );
    assert_eq!(
        single_components[0],
        vec![42],
    ...
    assert!(
        no_path.is_err(),
        "Should fail to find path between disconnected nodes"
    );
}
...
fn req4_performance_characteristics() {
    ...
    assert_eq!(
        path.unwrap(),
        vec![0, num_nodes],
        "Should find direct path from root to leaf"
    );
    ...
    assert!(
        ...
        "Algorithms should complete quickly on 100-node graph"
    );

    println!(
        "✅ Performance test completed in {:?} for {}-node graph",
        elapsed,
        num_nodes + 1
    );
}
