...
use mission8::{bfs, connected_components, dfs, has_cycle, shortest_path};
...
fn test_bfs_linear_time() {
    ...
    assert!(
        times[3] < times[0] * 15,
        "BFS should scale roughly linearly, got {:?}",
        times
    );
...
fn test_dfs_linear_time() {
    ...
    assert!(
        times[3] < times[0] * 15,
        "DFS should scale roughly linearly, got {:?}",
        times
    );
...
fn test_shortest_path_performance() {
    ...
    for &size in &sizes {
        ...
        let (duration, path) = measure_time(|| shortest_path(&graph, start_node, end_node));
...
fn test_cycle_detection_performance() {
    ...
    for &size in &sizes {
        ...
        let (duration, has_cycle_result) = measure_time(|| has_cycle(&cyclic_graph));
        ...
        let (duration2, has_cycle_result2) = measure_time(|| has_cycle(&acyclic_graph));
        ...
        // Should complete quickly
        assert!(
            duration2 < Duration::from_millis(50),
...
fn test_connected_components_performance() {
    ...
    for &size in &component_sizes {
        ...
        let (duration1, components1) = measure_time(|| connected_components(&single_graph));
        ...
        // Should complete quickly
        assert!(
            duration1 < Duration::from_millis(200),
            "Component finding on {} nodes took {:?}, should be under 200ms",
            size,
            duration1
        );
        ...
        let (duration2, components2) = measure_time(|| connected_components(&multi_graph));
        ...
        assert!(
            ...
            "Component finding on disconnected {} nodes took {:?}, should be under 200ms",
            size,
            duration2
        );
    }
...
fn test_large_graph_performance() {
    ...
    assert!(
        bfs_time < Duration::from_millis(500),
    ...
    assert!(
        dfs_time < Duration::from_millis(500),
    ...
    let (path_time, _) = measure_time(|| shortest_path(&graph, 0, large_size - 1));
    assert!(
        path_time < Duration::from_millis(100),
    ...
    assert!(
        cycle_time < Duration::from_millis(200),
    ...
    assert!(
        comp_time < Duration::from_millis(10000),
