...
fn analyze_network(name: &str, network: Network) {
    ...
    println!("📊 Topology: {} nodes, {} connections", 
             network.node_count(), network.connection_count());
    ...
    if components.len() == 1 {
        ...
        println!("  ⚠️  Network has {} isolated partitions:", components.len());
        ...
        for (&source, &target) in test_pairs {
            match shortest_path(network, source, target) {
                Ok(path) => {
                    ...
                    println!("  📤 {} → {}: {} hops", 
                             source_info, target_info, path.len() - 1);
                    println!("    Route: {}", 
                             path.iter()
...
fn calculate_network_efficiency(network: &Network) {
    ...
    if paths_calculated > 0 {
        ...
        println!("  🎯 Network diameter: {} (worst-case communication delay)", max_hops);
        
        // Network efficiency score (lower is better)
        let efficiency_score = avg_hops / nodes.len() as f64;
        println!("  ⚡ Efficiency score: {:.3} (lower = more efficient)", efficiency_score);
    }
}
...
fn analyze_network_robustness(network: &Network) {
    ...
    println!("  🔧 Original network: {} partition(s)", original_components);
    ...
    println!("  ⚠️  Critical single points of failure: {}/{}", 
             critical_failures, nodes.len());
    if critical_failures > 0 {
        println!("  💥 Worst case: {} partitions if critical node fails", 
...
fn find_critical_nodes(network: &Network) {
    ...
    if critical_nodes.is_empty() {
        ...
        for (node, connections, partitions_added) in critical_nodes {
            ...
            println!("    🔴 {}: {} connections, +{} partition(s) if removed", 
...
impl Network {
    ...
    pub fn add_node(&mut self, name: &str, node_type: NodeType) -> NetworkNode {
        ...
        self.node_info.insert(node_id, NodeInfo::new(name, node_type));
    ...
    pub fn connection_count(&self) -> usize {
        self.connections.values()
            ...
            .sum::<usize>() / 2  // Divide by 2 because each connection is stored twice
    }
