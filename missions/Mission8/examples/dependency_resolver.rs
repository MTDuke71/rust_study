...
fn analyze_dependencies(name: &str, deps: DependencyGraph) {
    ...
    println!("📊 Overview: {} packages, {} dependencies", 
             deps.package_count(), deps.dependency_count());
...
fn analyze_critical_packages(deps: &DependencyGraph) {
    ...
    let mut critical_packages: Vec<_> = dependents_count.iter()
...
fn analyze_dependency_depths(deps: &DependencyGraph) {
    ...
    // Analysis insights
    if max_depth > 5 {
        println!("  ⚠️  Deep dependency chains detected ({}+ levels)", max_depth);
...
fn calculate_dependency_depth(deps: &DependencyGraph, package: PackageId) -> usize {
    ...
    fn dfs_depth(
        ...
        let max_child_depth = deps.neighbors(current)
            .iter()
...
fn analyze_security_impact(deps: &DependencyGraph) {
    ...
    let vulnerable_packages: Vec<_> = packages.iter()
        .filter(|&&pkg| {
            let info = deps.get_package_info(pkg).unwrap();
            info.name.contains("old") || info.name.contains("legacy") || 
    ...
    if vulnerable_packages.is_empty() {
        ...
        for &vulnerable_pkg in &vulnerable_packages {
            ...
            println!("    ⚠️  {}: affects {} packages transitively", 
                     info.name, affected_packages.len());
...
fn suggest_build_order(deps: &DependencyGraph) {
    ...
    let mut dependency_counts: Vec<_> = packages.iter()
        .map(|&pkg| {
    ...
    for (i, (pkg, direct_deps, total_depth)) in dependency_counts.iter().enumerate() {
        let info = deps.get_package_info(*pkg).unwrap();
        println!("    {}. {} (deps: {}, depth: {})", 
                 i + 1, info.name, direct_deps, total_depth);
    }
...
impl DependencyGraph {
    ...
    pub fn add_package(&mut self, name: &str, version: &str, package_type: PackageType) -> PackageId {
        let package_id = self.next_id;
        self.next_id += 1;
        
        self.package_info.insert(package_id, PackageInfo::new(name, version, package_type));
