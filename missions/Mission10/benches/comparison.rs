...
fn bench_find_comparison(c: &mut Criterion) {
    ...
    for &size in COMPARISON_SIZES {
        ...
        group.bench_with_input(BenchmarkId::new("union_by_rank", size), &size, |b, &s| {
            b.iter(|| {
                for _ in 0..10 {
...
fn bench_memory_usage_comparison(c: &mut Criterion) {
    ...
    for &size in &[1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::new("naive_creation", size), &size, |b, &s| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("rank_creation", size), &size, |b, &s| {
            b.iter(|| {
