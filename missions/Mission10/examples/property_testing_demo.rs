...
fn demonstrate_union_commutativity() {
    ...
    println!("   ✅ Union order doesn't affect final connectivity: {}", identical);
...
fn demonstrate_find_idempotency() {
    ...
    println!("   ✅ Multiple find(0) calls return same root: {} (result: {})", idempotent, first_find);
...
fn demonstrate_transitivity() {
    ...
    println!("   🎯 Transitivity property holds: {}\n", ab && bc && ac && !ad);
}
...
fn demonstrate_count_property() {
    ...
    println!("   🔗 union(0, 2) - were already connected: {}", were_connected);
    println!("   📉 Components after union: {} (decreased by {})", 
    ...
    println!("   🔄 union(0, 2) again - already connected: {}", already_connected);
    println!("   📊 Components after redundant union: {} (no change)", after_redundant);
    println!("   ✅ Count property verified: {}\n", before_redundant == after_redundant);
}