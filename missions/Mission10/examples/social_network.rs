...
impl Person {
    pub fn new(id: usize, name: String, age: u32, interests: Vec<String>, location: String) -> Self {
        Self { id, name, age, interests, location }
    }
}
...
impl SocialNetwork {
    ...
    pub fn add_person(&mut self, name: String, age: u32, interests: Vec<String>, location: String) -> Result<usize, String> {
    ...
    pub fn add_friendship(&mut self, person1: usize, person2: usize, connection_type: ConnectionType) -> Result<bool, String> {
    ...
    pub fn suggest_friends(&mut self, person: usize, limit: usize) -> Result<Vec<&Person>, String> {
        ...
        for (id, candidate) in &self.people {
            ...
            let common_interests: HashSet<_> = person_obj.interests.iter()
    ...
    pub fn print_statistics(&mut self) -> Result<(), String> {
        ...
        if communities.len() > 1 {
            ...
            println!("   Largest community has {} people", largest_community.len());
        }
    ...
    pub fn print_communities(&mut self) -> Result<(), String> {
        ...
        for (i, community) in communities.iter().enumerate() {
            if community.len() == 1 {
                ...
                let avg_age: f32 = community.iter().map(|p| p.age as f32).sum::<f32>() / community.len() as f32;
                ...
                println!("      📍 Locations: {}", locations.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", "));
                
                if let Some((popular_interest, count)) = interest_counts.iter().max_by_key(|(_, v)| **v) {
                    if *count > 1 {
                        println!("      ⭐ Popular interest: {} ({} people)", popular_interest, count);
                    }
                }
            }
            println!();
    ...
    pub fn simulate_viral_spread(&mut self, starting_person: usize, spread_probability: f64) -> Result<Vec<usize>, String> {
        if !self.people.contains_key(&starting_person) {
            return Err("Starting person not found".to_string());
        }
        
        println!("🦠 Simulating viral spread starting from {}", 
        ...
        println!("   📊 Infected {} out of {} people in the community", 
                 infected.len(), friend_circle_size);
        
        for id in &infected[1..] { // Skip the starting person
...
fn example_university_network() -> Result<(), String> {
    ...
    let alice = network.add_person(
        "Alice".to_string(), 20, 
        vec!["Computer Science".to_string(), "Gaming".to_string(), "Anime".to_string()],
        "Dorm A".to_string()
    )?;
    
    let bob = network.add_person(
        "Bob".to_string(), 21,
        vec!["Computer Science".to_string(), "Basketball".to_string(), "Music".to_string()],
        "Dorm A".to_string()
    )?;
    
    let charlie = network.add_person(
        "Charlie".to_string(), 19,
        vec!["Mathematics".to_string(), "Gaming".to_string(), "Chess".to_string()],
        "Dorm B".to_string()
    )?;
    
    let diana = network.add_person(
        "Diana".to_string(), 22,
        vec!["Biology".to_string(), "Photography".to_string(), "Hiking".to_string()],
        "Off-campus".to_string()
    )?;
    
    let eve = network.add_person(
        "Eve".to_string(), 20,
        vec!["Physics".to_string(), "Anime".to_string(), "Chess".to_string()],
        "Dorm B".to_string()
    )?;
    
    let frank = network.add_person(
        "Frank".to_string(), 23,
        vec!["Computer Science".to_string(), "Photography".to_string(), "Basketball".to_string()],
        "Off-campus".to_string()
    )?;
    ...
    for (i, person) in suggestions.iter().enumerate() {
        println!("   {}. {} (interests: {})", 
                 i + 1, person, person.interests.join(", "));
    }
...
fn example_professional_network() -> Result<(), String> {
    ...
    let people_data = vec![
        ("Sarah", 28, vec!["Software Engineering".to_string(), "Machine Learning".to_string()], "Tech Corp"),
        ("Mike", 32, vec!["Product Management".to_string(), "Startups".to_string()], "StartupX"),
        ("Lisa", 25, vec!["UI/UX Design".to_string(), "User Research".to_string()], "Design Studio"),
        ("James", 30, vec!["Software Engineering".to_string(), "Blockchain".to_string()], "Tech Corp"),
        ("Anna", 27, vec!["Data Science".to_string(), "Machine Learning".to_string()], "AI Labs"),
        ("David", 35, vec!["Product Management".to_string(), "Enterprise".to_string()], "Big Corp"),
        ("Emma", 26, vec!["UI/UX Design".to_string(), "Mobile Apps".to_string()], "StartupX"),
        ("Ryan", 29, vec!["Data Science".to_string(), "Analytics".to_string()], "Analytics Co"),
    ];
    ...
    println!("• Can Sarah (Tech Corp) reach Emma (StartupX)? {}", 
    ...
    if network.are_friends(people_ids[0], people_ids[6])? {
        ...
        println!("  → Sarah's professional network size: {}", sarah_circle.len());
    }
...
fn example_community_growth() -> Result<(), String> {
    ...
    for i in 5..15 {
        ...
        for existing_id in &people {
            if let (Some(new_person), Some(existing_person)) = (network.get_person(id), network.get_person(*existing_id)) {
                let common_interests: Vec<_> = new_person.interests.iter()
    ...
    for i in 15..30 {
        ...
        for existing_id in &people {
            if connections >= 3 { break; } // Limit connections per person
    ...
    if communities.len() == 1 {
        ...
        println!("   Information reached {:.1}% of the network", 
                 (infected.len() as f64 / people.len() as f64) * 100.0);
    } else {
        println!("📊 Network formed {} separate communities", communities.len());
        for (i, community) in communities.iter().enumerate() {
...
mod tests {
    ...
    fn test_basic_friendship() {
        ...
        // Add friendship
        assert!(network.add_friendship(alice, bob, ConnectionType::Direct).unwrap());
    ...
    fn test_friend_circles() {
        ...
        let alice = network.add_person("Alice".to_string(), 25, vec![], "NYC".to_string()).unwrap();
        let bob = network.add_person("Bob".to_string(), 26, vec![], "NYC".to_string()).unwrap();
        let charlie = network.add_person("Charlie".to_string(), 27, vec![], "LA".to_string()).unwrap();
        
        // Create friendship: Alice <-> Bob
        network.add_friendship(alice, bob, ConnectionType::Direct).unwrap();
        ...
        network.add_friendship(alice, charlie, ConnectionType::Online).unwrap();
    ...
    fn test_communities() {
        ...
        let group1_a = network.add_person("A".to_string(), 25, vec![], "NYC".to_string()).unwrap();
        let group1_b = network.add_person("B".to_string(), 26, vec![], "NYC".to_string()).unwrap();
        
        let group2_c = network.add_person("C".to_string(), 27, vec![], "LA".to_string()).unwrap();
        let group2_d = network.add_person("D".to_string(), 28, vec![], "LA".to_string()).unwrap();
        
        // Connect within groups
        network.add_friendship(group1_a, group1_b, ConnectionType::Location).unwrap();
        network.add_friendship(group2_c, group2_d, ConnectionType::Location).unwrap();
    ...
    fn test_duplicate_friendship() {
        ...
        assert!(!network.add_friendship(alice, bob, ConnectionType::Interest).unwrap());
        assert!(!network.add_friendship(bob, alice, ConnectionType::Location).unwrap());
