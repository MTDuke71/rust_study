...
impl fmt::Display for Connection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "T{}: {} <-> {} (components: {})",
            self.timestamp, self.from_name, self.to_name, self.connected_components_after
        )
    }
}
...
impl DynamicNetwork {
    ...
    pub fn add_connection(&mut self, from: usize, to: usize) -> Result<bool, String> {
        ...
        if was_connected {
            println!(
                "  ℹ️  {} and {} were already connected",
                self.get_node_name(from)?,
                self.get_node_name(to)?
            );
        }
    ...
    pub fn simulate_partition_healing(&mut self) -> Result<(), String> {
        ...
        for i in 1..component_roots.len() {
            ...
            println!(
                "   🔗 Connecting {} to {} (healing partition)",
        ...
        println!(
            "   ✅ Network healing complete! Components: {}",
...
fn example_basic_connectivity() -> Result<(), String> {
    ...
    println!(
        "5. Can Laptop-1 reach Printer? {}",
        network.are_connected(laptop1, printer)?
    );

    println!(
        "6. Laptop-2 is still isolated. Can it reach Server? {}",
    ...
    println!(
        "8. Now can Laptop-2 reach Server? {}",
        network.are_connected(laptop2, server)?
    );
...
fn example_social_network() -> Result<(), String> {
    ...
    println!(
        "• Can Alice reach Charlie through mutual friends? {}",
        network.are_connected(alice, charlie)?
    );
    println!(
        "• Can Alice reach Diana? {}",
        network.are_connected(alice, diana)?
    );
    println!(
        "• Size of Alice's friend network: {}",
...
fn example_network_partition() -> Result<(), String> {
    ...
    println!(
        "• Can DataCenter-A reach DataCenter-B? {}",
        network.are_connected(nodes[0], nodes[1])?
    );
    println!(
        "• Can Office-NYC reach Office-LA? {}",
    ...
    println!(
        "• Can Edge-4 (previously isolated) reach DataCenter-A? {}",
...
fn example_large_network_performance() -> Result<(), String> {
    ...
    println!(
        "⏱️  Adding {} connections took: {:?}",
        num_connections, connection_time
    );
    ...
    println!(
        "⏱️  {} connectivity queries took: {:?}",
        num_queries, query_time
    );
    println!(
        "📊 Query results: {}/{} pairs were connected",
        connected_count, num_queries
    );
    println!(
