use mission6::{Coord, Direction, Grid, GridVisualizer};
...
fn basic_grid_operations() {
    ...
    println!("\nBounds checking:");
    println!(
        "Is (3,2) valid in 4x3 grid? {}",
        char_grid.in_bounds(Coord::new(3, 2))
    );
    println!(
        "Is (4,2) valid in 4x3 grid? {}",
        char_grid.in_bounds(Coord::new(4, 2))
    );
...
fn coordinate_system_demo() {
    ...
    println!(
        "Manhattan distance: {}",
        corner1.manhattan_distance(corner2)
    );
    println!(
        "Euclidean distance: {:.2}",
        (corner1.euclidean_distance_squared(corner2) as f64).sqrt()
    );
    println!(
        "Chebyshev distance: {}",
        corner1.chebyshev_distance(corner2)
    );

    // Test coordinate as signed
    let signed = center.to_signed();
    println!(
        "\nCenter as signed coordinates: ({}, {})",
        signed.0, signed.1
    );
}
...
fn advanced_indexing() {
    ...
    println!(
        "Via Coord: grid[{:?}] = {}",
        center_coord, grid[center_coord]
    );
    ...
    println!(
        "Parsed '{}' as {:?}, value = {}",
        coord_str, parsed_coord, grid[parsed_coord]
    );
}
...
fn grid_transformations() {
    ...
    println!(
        "{}",
        GridVisualizer::highlight_coords(&large_grid, &important_coords, '*')
    );
}
