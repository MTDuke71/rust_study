use mission6::{Grid, Coord, Direction, PathFinder, FloodFill, AocGridParser, GridVisualizer};
...
fn demo_grid_creation() {
    ...
    println!("Is (4,4) in bounds for 5x5 grid? {}", char_grid.in_bounds(Coord::new(4, 4)));
    println!("Is (5,5) in bounds for 5x5 grid? {}", char_grid.in_bounds(Coord::new(5, 5)));
}
...
fn demo_coordinate_navigation() {
    ...
    println!("Manhattan distance to north point: {}", center.manhattan_distance(north));
    println!("Euclidean distance to northeast point: {:.2}", (center.euclidean_distance_squared(northeast) as f64).sqrt());
    ...
    println!("4-connected neighbors within 15x15 grid: {} total", bounded_4.len());
}
...
";
    ...
    println!("Region analysis: {} cells, {} perimeter", result.area, result.perimeter);
...
fn demo_performance() {
    ...
    println!("Coordinate enumeration time: {:?} ({} coords)", coord_time, coord_count);
    
    // Memory usage approximation
    let memory_bytes = grid.len() * std::mem::size_of::<u32>();
    println!("Approximate memory usage: {} bytes ({:.2} MB)", 
             memory_bytes, memory_bytes as f64 / 1_000_000.0);
}
...
fn demo_integration_patterns() {
    ...
    while let Some(current) = queue.pop_front() {
        steps += 1;
        if steps > 10 { break; } // Limit for demo
