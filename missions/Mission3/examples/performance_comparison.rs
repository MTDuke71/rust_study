...
fn compare_basic_search(data: &[i32]) {
    ...
    println!("    std library:      {:>8} ns/op", std_time.as_nanos() / iterations);
    println!("    our implementation: {:>8} ns/op", our_time.as_nanos() / iterations);
...
fn compare_bounds_search(data: &[i32]) {
    ...
    println!("    left+right bounds: {:>8} ns/op", bounds_time.as_nanos() / iterations);
    ...
    println!("    2x basic search:   {:>8} ns/op", double_search_time.as_nanos() / iterations);
...
fn test_iterator_performance() {
    ...
    for i in 0..10000 {
        for _ in 0..5 { // 5 copies of each number
            data.push(i);
        }
    }
    ...
    println!("    Binary search iter: {:>8} ns/op", iter_time.as_nanos() / iterations);
    println!("    Linear filter:      {:>8} ns/op", linear_time.as_nanos() / iterations);
...
fn test_trait_overhead() {
    ...
    println!("    direct slice:    {:>8} ns/op", direct_time.as_nanos() / iterations);
    println!("    trait-based:     {:>8} ns/op", trait_time.as_nanos() / iterations);
    ...
    println!("    by-key search:   {:>8} ns/op", key_time.as_nanos() / iterations);
