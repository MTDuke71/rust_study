...
fn coordinate_grid_search() {
    ...
    use aoc_utils::{find_closest_coord, find_coords_within_distance, Coord, Grid};
    ...
    println!(
        "Sensor at {:?} detects {} points within range {}",
        sensor_pos,
        detected.len(),
        sensor_range
    );
    ...
    if let Some(closest) = find_closest_coord(&points, player_pos) {
        ...
        println!(
            "Closest point to player at {:?} is {:?} (distance: {})",
...
fn range_query_puzzles() {
    ...
    println!(
        "Terrain distribution: {} low, {} mid, {} high",
...
fn multi_criteria_item_search() {
    ...
    match find_item_by_value(&items, 150) {
        Ok(idx) => println!(
            "Found item worth 150: #{} ({})",
            items[idx].id, items[idx].category
        ),
        Err(idx) => println!(
            "No item worth exactly 150, would insert at position {}",
            idx
        ),
    }
...
fn pathfinding_coordinate_support() {
    ...
    use aoc_utils::{find_coords_within_distance, Coord};
    ...
    println!(
        "\nFrom start {:?}, reachable within {} moves:",
        start, movement_range
    );
    for wp in reachable {
    ...
    println!(
        "\nNear checkpoint {:?} (range {}):",
        checkpoint, interaction_range
    );
    for wp in nearby {
