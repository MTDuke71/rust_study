...
fn req2_custom_container() {
    use searchable::{search_in_by_key, search_in_by};
    ...
    assert_eq!(
        search_in_by(&people, &(0, "Bob"), |person, target| person.1.cmp(&target.1)),
        Ok(1)
    );
}
...
fn req4_complex_ordering() {
    ...
    // Search by age
    assert_eq!(
        binary_search::search_by_key(&people, &30, |p| p.age),
        Ok(1)
    );
...
fn req6_coordinate_search() {
    use aoc_utils::{Coord, find_coords_within_distance, find_closest_coord};
...
fn property_test_against_std_library() {
    ...
    for _ in 0..100 {
        ...
        match (std_result, our_result) {
            ...
            _ => panic!("Results should match: std={:?}, ours={:?}", std_result, our_result),
        }
    }
}