...
fn analyze_cache_behavior(n: u64) {
    ...
    println!("- Coverage: {:.1}%", (stats.max_size as f64 / (n + 1) as f64) * 100.0);
    println!("- Lookups per cache entry: {:.2}", stats.lookups as f64 / stats.max_size as f64);
    
    if stats.is_memoization_beneficial() {
        println!("\n✅ Memoization is HIGHLY beneficial!");
        println!("   Hit ratio {:.1}% indicates significant subproblem overlap", stats.hit_ratio() * 100.0);
    } else {
