...
fn demo_basic_pattern_matching() {
    ...
    for (design, expected) in test_cases {
        ...
        let status = if result { "✅ Possible" } else { "❌ Impossible" };
        println!("  Design: {:10} → {} (cached: {} states, took: {:?})",
            design, status, memo.len(), elapsed);
...
fn demo_counting_arrangements() {
    ...
    for design in designs {
        ...
        println!("  Design: {:10} → {} arrangements (cached: {} states, took: {:?})",
            design, count, memo.len(), elapsed);
    }
...
fn demo_performance_comparison() {
    ...
    for length in [5, 10, 15, 20] {
        ...
        println!("  Target length: {:2} → Result: {:5}, Cache size: {:3}, Time: {:?}",
            length, result, memo.len(), elapsed);
    }
...
fn demo_aoc_day19_simulation() {
    ...
    println!("  Total cache entries across all designs: {}\n", part1_cache_total);
    ...
    for design in &desired_designs {
        ...
        if count > 0 {
            ...
            println!("  {} → {} arrangements (cached {} states)", 
                design, count, memo.len());
        }
    }
    
    println!("\n  Part 2 Answer: {} total arrangements", total_arrangements);
    println!("  Average cache size per design: {:.1}\n", 
...
fn demo_transformation_pattern() {
    ...
    println!("\n  Counting DP: Ways to construct \"{}\"? {}", target, count);
