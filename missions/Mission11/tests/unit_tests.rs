...
use mission11::string_dp::{can_construct, can_construct_multi, count_constructions};
...
fn req1_zero_copy_string_slice_keys() {
    ...
    fn count_arrangements<'a>(pattern: &'a str, memo: &mut HashMap<&'a str, usize>) -> usize {
        if pattern.is_empty() {
            return 1; // Base case
        }
...
fn test_aoc_day12_pattern_integration() {
    ...
    fn count_arrangements(state: State, max_pos: usize, memo: &mut HashMap<State, usize>) -> usize {
...
fn req3_lifetime_safety() {
    ...
    fn helper<'a>(s: &'a str, patterns: &[&str]) -> HashMap<&'a str, bool> {
        let mut memo = HashMap::new();
...
fn req3_no_to_string_in_hot_path() {
    ...
    for key in memo.keys() {
        ...
        assert!(
            std::ptr::eq(target.as_ptr(), key.as_ptr()) || key.is_empty() || target.contains(*key)
        );
    }
}
...
fn req5_pattern5_custom_struct_state() {
    use mission11::state_patterns::{custom_struct_pattern, GameState};
...
fn req5_advanced_composite_with_borrowed() {
    ...
    let pattern_groups = vec![vec!["a", "ab"], vec!["c", "cd"]];
    let mut memo = HashMap::new();

    assert!(composite_with_borrowed(
        "abc",
        0,
        &pattern_groups,
        &mut memo
