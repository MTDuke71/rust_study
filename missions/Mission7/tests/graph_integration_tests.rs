...
use mission7::{Graph, GraphType, utils};
...
fn req3_graph_construction() {
    ...
    // Add nodes
    let nodes: Vec<_> = "ABCDEF".chars()
        .map(|c| graph.add_node(c))
        .collect();
    
    // Define edges
    let edges = [
        (0, 1), (0, 2),  // A connects to B, C
        (1, 3), (1, 4),  // B connects to D, E
        (2, 4), (2, 5),  // C connects to E, F
        (3, 4),          // D connects to E
    ];
...
fn req3_components() {
    ...
    assert!(components.iter().any(|comp| comp.len() == 1 && comp.contains(&f)));
}
...
fn req4_path_reconstruction() {
    ...
    assert_eq!(path, Some(vec![nodes[0], nodes[1], nodes[2], nodes[3], nodes[4]]));
}
