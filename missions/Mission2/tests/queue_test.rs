use mission2::queue::{LinkedQueue, RingBufferQueue};
    ...
    for i in 0..10_000u32 {
        if (i.wrapping_mul(2654435761u32)) % 7 != 0 {
            ...
            if v.len() < q.capacity() {
                ...
                assert!(
                    ring_result.is_ok(),
                    "Ring buffer should accept value when VecDeque has space"
                );
                v.push_back(x);
            } else {
                ...
                assert_eq!(
                    ring_result,
                    Err(x),
                    "Ring buffer should reject when at capacity"
                );
            }
        } else {
            ...
            assert_eq!(
                ring_result, vec_result,
                "Ring buffer and VecDeque should match"
            );
        }
...
fn performance_characteristics() {
    ...
    assert!(
        linked_time.as_millis() < 1000,
        "Linked queue should be fast"
    );
    ...
    for run in 0..3 {
        ...
        for &(is_enqueue, value) in &operations {
            if is_enqueue {
                ...
                assert_eq!(
                    ring_result, expected_result,
                    "Run {}: Ring buffer determinism",
                    run
                );
                assert_eq!(
                    linked_result, expected_result,
                    "Run {}: Linked queue determinism",
                    run
                );
            }
        }
