use mission6::{Grid, Coord, Direction, PathFinder, FloodFill, AocGridParser, AocAlgorithms};
...
fn santa_delivery_simulation() {
    ...
    println!("Santa moved within bounds: x=[{}, {}], y=[{}, {}]", min_x, max_x, min_y, max_y);
...
.......#####..";
    ...
    for (i, island) in islands.iter().enumerate() {
        if let Some(&first_cell) = island.first() {
            ...
            println!("  Island {}: area={}, perimeter={}", 
                     i + 1, analysis.area, analysis.perimeter);
        }
    }
}
...
fn cellular_automaton_simulation() {
    ...
    for step in 1..=5 {
        ...
        // Check for stability
        let live_cells: usize = current_grid.iter().map(|&alive| if alive { 1 } else { 0 }).sum();
    ...
    if let Some((cycle_start, cycle_length)) = AocAlgorithms::detect_cycle(
        current_grid,
        life_step,
        20
    ) {
        println!("\nCycle detected: starts at step {}, length {}", cycle_start, cycle_length);
    } else {
