...
impl AocAlgorithms {
    ...
    {
        if use_diagonals {
            coord.neighbors_8_bounded(grid.width(), grid.height())
                .filter(|&neighbor| predicate(&grid[neighbor]))
                .count()
        } else {
            coord.neighbors_4_bounded(grid.width(), grid.height())
                .filter(|&neighbor| predicate(&grid[neighbor]))
                .count()
        }
    }
    ...
    pub fn apply_cellular_automaton<T, F>(
        grid: &Grid<T>,
        rule: F,
    ) -> Grid<T>
    where
    ...
    pub fn multi_source_bfs<T, F>(
        ...
        sources: &[Coord],
        is_passable: F,
    ) -> HashMap<Coord, (usize, Coord)>  // (distance, closest_source)
    where
        F: Fn(&T) -> bool,
    {
    ...
    pub fn detect_cycle<T: Clone + PartialEq + std::hash::Hash + Eq>(
        ...
        max_steps: usize,
    ) -> Option<(usize, usize)> { // (cycle_start, cycle_length)
...
mod tests {
    ...
    fn test_count_adjacent() {
        ...
        let count = AocAlgorithms::count_adjacent(
            &grid,
            Coord::new(1, 1),
            false,
            |&ch| ch == '#'
